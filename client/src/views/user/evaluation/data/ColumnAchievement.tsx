...
const AchievementColumn = ({
  ...
  const { Item } = Form;
  const reduceDatas = dataSources.reduce((pre, curr, index) => ({ ...pre, [index]: curr }), {});
  ...
  useEffect(() => {
    ...
    if (convertDataList) dispatch(userEvaluationAchievement(convertDataList));
    if (convertDataList.length > 0) setDataSource(convertDataList);
    convertDataList.forEach((val: any) => {
      setExpandedRowKey((state) => [...state, val.key]);
    });
            ...
            >
              <TextArea
                ...
                onChange={(e) => {
                  ...
                  startTransition(() => setEvaluationAchievement({ ...userEvaluationAchievements }));
                }}
              />
            </Item>
          ) : (
    ...
    },
    {
      ...
      render: (text, record, index) => (
        <>
          ...
          {!isHiddenInput ? (
            <Item
              name={`achievementValue-key-${record.key}`}
              initialValue={text}
              rules={[
                { type: 'string' },
                { required: true, message: t('MESSAGE.COMMON.IDM_BLANK_ITEM') as string },
              ...
              <Input
                ...
                onChange={(e) => {
                  ...
                  startTransition(() => {
                    ...
                    setEvaluationAchievement({ ...userEvaluationAchievements });
                  });
                }}
              />
            </Item>
...

    {
      ...
      render: (text, record, index) => (
        <>
          ...
          {!isHiddenInput ? (
            <Item
              name={`difficultyUser-key-${record.key}`}
              initialValue={text}
        ...
        <>
          ...
          <Button
            ...
            onClick={() => {
              ...
              Object.keys(userEvaluationAchievements).some((s: any) => {
                if (userEvaluationAchievements[s].key === record.key) return (key = s);
              });

              if (key) {
                // delete userEvaluationAchievements[key as any];

                // const converObj: any = Object.values(userEvaluationAchievements).reduce(
    ...
    (columns[indexParent] as { children: any[] }).children.push({
      ...
      render: (text: string, record: any) => {
        if (isEditEvaluation05)
          return (
            <Item
              name={`difficultyUser05-key-${record.key}`}
              initialValue={text}
              rules={[{ required: true, message: t('MESSAGE.COMMON.IDM_BLANK_ITEM') as string }]}
              style={{ textAlign: 'center', margin: 0 }}
            >
    ...
    (columns[indexParent] as { children: any[] }).children.push({
      ...
      render: (text: string, record: any) => {
        if (isEditEvaluation1)
          return (
            <Item
              name={`difficultyUser1-key-${record.key}`}
              initialValue={text}
              rules={[{ required: true, message: t('MESSAGE.COMMON.IDM_BLANK_ITEM') as string }]}
              style={{ textAlign: 'center', margin: 0 }}
            >
    ...
    (columns[indexParent] as { children: any[] }).children.push({
      ...
      render: (text: string, record: any) => {
        if (isEditEvaluation2)
          return (
            <Item
              name={`difficultyUser3-key-${record.key}`}
              initialValue={text}
              rules={[{ required: true, message: t('MESSAGE.COMMON.IDM_BLANK_ITEM') as string }]}
              style={{ textAlign: 'center', margin: 0 }}
            >
  ...
  const columns: ColumnsType<any[]> = [
    {
      ...
      render: (text, record: any, i) => {
        if (!isHiddenInput) {
          return (
            <Item
              name={`note-key-${record.key}-${key}-${i}`}
              initialValue={text}
              rules={[
                { type: 'string' },
                { required: true, message: t('MESSAGE.COMMON.IDM_BLANK_ITEM') as string },
...
export default AchievementColumn;
