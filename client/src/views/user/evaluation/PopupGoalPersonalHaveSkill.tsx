...
import {
  ...
  userEvaluationCloneAchievement2,
...
const PopupGoalPersonalHaveSkill = (props: Props) => {
  const {
    isOpen,
    title,
    t,
  ...
  const addRows = () => {
    if (store.achievementDatas && store.achievementDatas.length + selectedRowsKeys.length <= 30) {
      ...
      for (let index = 0; index < filterDataSouces.length; index++) {
        ...
        items.push({
          ...itemRow,
          key: `achievement-component-key-${filterDataSouces[index].id + 1 + index}-${random}`,
          itemNo: store.achievementDatas.length + index,
          ...
          evaluationAchievementPersonalSub: achievementSubs.map((v, i) => ({
      ...
      const merges =
        store.achievementDatas.length > 0
          ? ([...store.achievementDatas, ...items] as UserEvaluationAchievementType[])
          : ([...dataStates, ...items] as UserEvaluationAchievementType[]);
      const objectValues: any[] = [];
      if (Object.values(store.achievementSubs).flat().length > 0) {
        [
          ...[Object.values(store.achievementSubs).map((v) => v)][0],
          ...items.map((v) => v.evaluationAchievementPersonalSub),
        ].map((val, i) => {
        ...
        [...dataStates, ...items.map((v) => v.evaluationAchievementPersonalSub)].map((val: any, i) => {
          if (val[i]) {
            dispatch(
              setAchievementSubRedux({ [val[i].index]: val.map((value: any) => ({ ...value, index: value.index })) }),
            );
          } else {
            dispatch(
              setAchievementSubRedux({
                [i]: achievementSubs.map((value: any) => ({
                  index: i,
                  degree: value.degree,
                  point: String(value.coefficient),
      ...
      setDataState(merges);
      setSelectRowsKeys([]);
      setPersonalGoalsList(merges);
      dispatch(userEvaluationPersonalGoalsList(merges));
      dispatch(userEvaluationAchievement(merges));

      // dispatch(userEvaluationCloneAchievement2([...items]));
...
export default PopupGoalPersonalHaveSkill;
