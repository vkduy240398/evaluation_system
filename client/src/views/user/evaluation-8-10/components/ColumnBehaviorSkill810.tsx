...
import {
  ...
  userEvaluationSetBehaviorSkillScore05,
  userEvaluationSetBehaviorSkillScore1,
  userEvaluationSetBehaviorSkillScore2,
...
const behaviorSkillColumn810 = ({
  ...
  const options = store.behaviorSkillPointOptions;

  const maxOptionScore =
    store.behaviorSkillPointOptions?.length > 0
      ? Math.max(...store.behaviorSkillPointOptions.map((v) => Number(v.value)))
      : 0;

  const dispatch = useDispatch<AppDispatch>();
  const evaluationBehaviorSkills = store.evaluationBehaviorSkills?.map((v) => ({ ...v }));
  const widthColumn = (isEdit: boolean) => (!isEdit ? 85 : 135);

  // ** Effect
  useEffect(() => {
    if (evaluationBehaviorSkills.length > 0) {
      dispatch(userEvaluationSetBehaviorSkillScoreUser(handleTotal('pointUser')));
      dispatch(userEvaluationSetBehaviorSkillScore05(handleTotal('pointEvaluator05')));
      dispatch(userEvaluationSetBehaviorSkillScore1(handleTotal('pointEvaluator1')));
      dispatch(userEvaluationSetBehaviorSkillScore2(handleTotal('pointEvaluator2')));
    }
  ...
  const handleTotal = (key: KeyObject) => {
    if (evaluationBehaviorSkills.filter((e) => e?.[key] !== null && e?.[key] !== undefined).length <= 0) return null;
    const totalScore = evaluationBehaviorSkills
      .filter((e) => e?.[key] !== null && e?.[key] !== undefined)
      .reduce((pre: number, cur) => pre + Number((cur?.[key] || 0) * cur.difficulty) || 0, 0);
    const maxTotalScore = evaluationBehaviorSkills.reduce(
      (pre: number, cur) => pre + Number(maxOptionScore * cur.difficulty) || 0,
      0,
    );
    const calculatorScore = isFinite(Math.round((totalScore / maxTotalScore) * 100))
      ...
      render: (text: any, record: BasicBehaviorSkillType, index: any) => {
        if (isEditUserEvaluation) {
          ...
          return (
            <Row align={'middle'}>
              <Col
                span={checkPointIs0(evaluationBehaviorSkills?.[index]?.pointUser) ? SPAN_ITEM : SPAN_ITEM_NO_ALERT}
                offset={SPAN_OFFSET}
              >
                <Item
                  rules={[
                  ...
                  name={`pointEvaluator-key${record.key}`}
                  initialValue={text}
                >
                  <Select
                    allowClear
              ...
              <Col span={checkPointIs0(evaluationBehaviorSkills?.[index]?.pointUser) ? SPAN_ICON : SPAN_ICON_NO_ALERT}>
                {evaluationBehaviorSkills?.[index]?.pointUser === 0 && displayAlert0Point}
              </Col>
              <Col
                span={checkPointIs0(evaluationBehaviorSkills?.[index]?.pointUser) ? SPAN_POINT : SPAN_POINT_NO_ALERT}
              >
          ...
          return (
            <Row align={'middle'}>
              <Col
                offset={SPAN_OFFSET}
                span={
                  checkPointIs0(evaluationBehaviorSkills?.[index]?.pointEvaluator05) ? SPAN_ITEM : SPAN_ITEM_NO_ALERT
                }
              >
                <Item
                  rules={[
              ...
              <Col
                span={
                  checkPointIs0(evaluationBehaviorSkills?.[index]?.pointEvaluator05) ? SPAN_ICON : SPAN_ICON_NO_ALERT
                }
              >
                {evaluationBehaviorSkills?.[index]?.pointEvaluator05 === 0 && displayAlert0Point}
              </Col>
              <Col
                span={
                  checkPointIs0(evaluationBehaviorSkills?.[index]?.pointEvaluator05) ? SPAN_POINT : SPAN_POINT_NO_ALERT
                }
              >
                <Typography style={{ fontWeight: 600 }}>
          ...
          return (
            <Row align={'middle'}>
              <Col
                offset={SPAN_OFFSET}
                span={
                  checkPointIs0(evaluationBehaviorSkills?.[index]?.pointEvaluator1) ? SPAN_ITEM : SPAN_ITEM_NO_ALERT
                }
              >
                <Item
                  rules={[
              ...
              <Col
                span={
                  checkPointIs0(evaluationBehaviorSkills?.[index]?.pointEvaluator1) ? SPAN_ICON : SPAN_ICON_NO_ALERT
                }
              >
                {evaluationBehaviorSkills?.[index]?.pointEvaluator1 === 0 && displayAlert0Point}
              </Col>
              <Col
                span={
                  checkPointIs0(evaluationBehaviorSkills?.[index]?.pointEvaluator1) ? SPAN_POINT : SPAN_POINT_NO_ALERT
                }
              >
                <Typography style={{ fontWeight: 600 }}>
          ...
          return (
            <Row align={'middle'}>
              <Col
                offset={SPAN_OFFSET}
                span={
                  checkPointIs0(evaluationBehaviorSkills?.[index]?.pointEvaluator2) ? SPAN_ITEM : SPAN_ITEM_NO_ALERT
                }
              >
                <Item
                  rules={[
              ...
              <Col
                span={
                  checkPointIs0(evaluationBehaviorSkills?.[index]?.pointEvaluator2) ? SPAN_ICON : SPAN_ICON_NO_ALERT
                }
              >
                {evaluationBehaviorSkills?.[index]?.pointEvaluator2 === 0 && displayAlert0Point}
              </Col>
              <Col
                span={
                  checkPointIs0(evaluationBehaviorSkills?.[index]?.pointEvaluator2) ? SPAN_POINT : SPAN_POINT_NO_ALERT
                }
              >
                <Typography style={{ fontWeight: 600 }}>
...
export default behaviorSkillColumn810;
