...
const expandedAchievementColumn810 = ({
  ...
  const shareOnCell3 = (record: UserEvaluationAchievementType) => {
    if (isColSpan(record.title) && record.key === -1)
  ...
  const shareOnCell = (record: UserEvaluationAchievementType) => {
    if (isColSpan(record.title) && record.key === -1) return { colSpan: 0, style: { verticalAlign: 'top' } };

    return { style: { verticalAlign: 'top' } };
  };

  const onCell = (record: any) => ({
    style: { verticalAlign: 'top', backgroundColor: record.key === -1 ? '#59ad5b78' : '' },
  });
  ...
  const columns: ColumnsType<UserEvaluationAchievementType> = [
    {
      ...
      render: (text, record, index) => (
        <>
          ...
          {isColSpan(record.title) && record.key === -1 ? (
            <>{text}</>
          ) : isEditUserEvaluation ? (
            <Item
              rules={[{ required: true, message: t('MESSAGE.COMMON.IDM_BLANK_ITEM') as string }]}
              name={`achievement-personal-achievementStatus-key${record.key}`}
    ...
    },
    {
      ...
      render: (text, record, index) => (
        <>
          ...
          {isEditUserEvaluation ? (
            <Item
              name={`achievement-personal-reasonComment-key-${record.key}`}
              initialValue={text}
              rules={[
                { type: 'string' },
            ...
            >
              <TextArea
                ...
                maxLength={1001}
                onFocus={(e) => {
                  initialValuesRef.current[`achievement-personal-reasonComment-key-${record.key}`] = e.target.value; // Lưu giá trị ban đầu theo key
                }}
                onBlur={(e) => {
                  const newValue = e.target.value ?? '';
                  const oldValue =
                    initialValuesRef.current[`achievement-personal-reasonComment-key-${record.key}`] ?? '';
    ...
    },
    {
      ...
      render: (text, record, index) => (
        <>
          ...
          {isEditUserEvaluation ? (
            <Item
              name={`achievement-personal-actionPlan-key-${record.key}`}
              initialValue={text}
              rules={[
                { type: 'string' },
                {
            ...
            >
              <TextArea
                ...
                maxLength={1001}
                onFocus={(e) => {
                  initialValuesRef.current[`achievement-personal-actionPlan-key-${record.key}`] = e.target.value; // Lưu giá trị ban đầu theo key
                }}
                onBlur={(e) => {
                  const newValue = e.target.value ?? '';
                  const oldValue = initialValuesRef.current[`achievement-personal-actionPlan-key-${record.key}`] ?? '';
                  if (newValue !== oldValue) {
                    startTransition(() => {
            ...
            isColSpan(record.title) && record.key === -1 ? (
              ...
              <Row align="middle">
                <Col span={calculateSpanAlert0Point(userEvaluationAchievements[index]?.pointUser)}>
                  <Item
                    name={`achievement-personal-pointUser-key-${record.key}`}
                  ...
                  >
                    <Input
                      ...
                      onFocus={(e) => {
                        initialValuesRef.current[`achievement-personal-pointUser-key-${record.key}`] = e.target.value; // Lưu giá trị ban đầu theo key
                      }}
                      onBlur={(e) => {
                        const newValue = e.target.value ?? '';
                        const oldValue =
                          initialValuesRef.current[`achievement-personal-pointUser-key-${record.key}`] ?? '';
                        if (newValue !== oldValue) {
                          startTransition(() => {
            ...
            isColSpan(record.title) && record.key === -1 ? (
              ...
              <Row align="middle">
                <Col span={calculateSpanAlert0Point(userEvaluationAchievements[index]?.pointEvaluator05)}>
                  <Item
                    name={`achievement-personal-point05-key-${record.key}`}
                    style={{ textAlign: 'left', margin: 0, width: '100%' }}
                    initialValue={text}
                    rules={[{ validator }]}
                  >
                    <Input
                      ...
                      style={{ textAlign: 'center' }}
                      onFocus={(e) => {
                        initialValuesRef.current[`achievement-personal-point05-key-${record.key}`] = e.target.value; // Lưu giá trị ban đầu theo key
                      }}
                      onBlur={(e) => {
                        const newValue = e.target.value ?? '';
                        const oldValue =
                          initialValuesRef.current[`achievement-personal-point05-key-${record.key}`] ?? '';
                        if (newValue !== oldValue) {
                          startTransition(() => {
            ...
            isColSpan(record.title) && record.key === -1 ? (
              ...
              <Row align="middle">
                <Col span={calculateSpanAlert0Point(userEvaluationAchievements[index]?.pointEvaluator1)}>
                  <Item
                    name={`achievement-personal-point1-key-${record.key}`}
                    style={{ textAlign: 'left', margin: 0, width: '100%' }}
                    initialValue={text}
                    rules={[{ validator }]}
                  >
                    <Input
                      ...
                      style={{ textAlign: 'center' }}
                      onFocus={(e) => {
                        initialValuesRef.current[`achievement-personal-point1-key-${record.key}`] = e.target.value; // Lưu giá trị ban đầu theo key
                      }}
                      onBlur={(e) => {
                        const newValue = e.target.value ?? '';
                        const oldValue =
                          initialValuesRef.current[`achievement-personal-point1-key-${record.key}`] ?? '';
                        if (newValue !== oldValue) {
                          startTransition(() => {
            ...
            isColSpan(record.title) && record.key === -1 ? (
              ...
              <Row align="middle">
                <Col span={calculateSpanAlert0Point(userEvaluationAchievements[index]?.pointEvaluator2)}>
                  <Item
                    name={`achievement-personal-point2-key-${record.key}`}
                    style={{ textAlign: 'left', margin: 0, width: '100%' }}
                    initialValue={text}
                    rules={[{ validator }]}
                  >
                    <Input
                      ...
                      style={{ textAlign: 'center' }}
                      onFocus={(e) => {
                        initialValuesRef.current[`achievement-personal-point2-key-${record.key}`] = e.target.value; // Lưu giá trị ban đầu theo key
                      }}
                      onBlur={(e) => {
                        const newValue = e.target.value ?? '';
                        const oldValue =
                          initialValuesRef.current[`achievement-personal-point2-key-${record.key}`] ?? '';
                        if (newValue !== oldValue) {
                          startTransition(() => {
...
export default expandedAchievementColumn810;
