...
const safeCompare = (val1: any, val2: any): boolean =>
...
const ColoredSelect = ({ color: _color, ...props }: any) => (
  <ConfigProvider theme={{ components: { Select: { borderRadius: 10 } } }}>
    <Select {...props} />
  </ConfigProvider>
);
...
const ImpactSection: React.FC<{
  ...
  border?: string;
  marginBottom?: number;
}> = ({ title, children, border = '1px solid #e5e7eb', marginBottom = 10 }) => (
  <div style={{ border, borderRadius: 8, marginBottom, overflow: 'hidden' }}>
    <div style={{
      padding: '6px 12px', backgroundColor: '#f3f4f6', borderBottom: '1px solid #e5e7eb',
      fontSize: FONT_SIZE, fontWeight: 600, color: '#374151',
    }}>
      {title}
    </div>
...
const Step3ConfirmDetail: React.FC<Step3ConfirmDetailProps> = React.memo(({
  ...
  const infoRows = useMemo(
    () => (current ? parseUserInfoChange(current.userInforChange) : []),
    [current],
  );
  ...
  const tableColumns = useMemo(() => [
    ...
    },
    {
      ...
      width: '45%',
      render: (val: string) => (
        <span style={{ color: val ? '#2c2a2a' : undefined, fontWeight: val ? 600 : undefined, fontSize: FONT_SIZE }}>
          {val || '変更しない'}
        </span>
      ),
    },
  ...
  return (
    <div style={{ display: 'flex', flex: 1, minHeight: 0 }}>
      ...
      {isMultiUser && (
        <div style={{
          minWidth: 200, borderRight: '1px solid #e5e7eb', display: 'flex',
          flexDirection: 'column', overflow: 'hidden', flexShrink: 0, minHeight: 0,
        }}>
              ...
              dataChanges.map((user, index) => (
                <div
                  ...
                  style={{
                    ...
                    borderLeft: index === selectedUserIndex ? '3px solid #047857' : '3px solid transparent',
                    transition: 'background-color 0.15s',
                  }}
                >
                  <div style={{ fontSize: FONT_SIZE, fontWeight: 500, color: '#1f2937' }}>{user.fullName}</div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minHeight: 0 }}>
        {isLoading ? (
          ...
          <>
            <div style={{
              padding: '10px 16px', borderBottom: '1px solid #e5e7eb', flexShrink: 0,
              fontSize: FONT_SIZE, display: 'flex', justifyContent: 'space-between', alignItems: 'center',
            }}>
              <div>
                <span style={{ color: '#374151' }}>{t('MODAL_EDIT_USER.IDS_TITLE_HEADING_MODAL_USER')}:&nbsp;</span>
              ...
              {targetMode !== '' && (
                <Tag style={{ fontSize: FONT_SIZE, margin: 0, background: '#FFFBEB', color: '#92400E', fontWeight: 600 }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '12px 16px' }}>
              ...
              <div>
                <div style={{
                  ...
                  marginBottom: 10, fontSize: FONT_SIZE, fontWeight: 600, color: '#92400e',
                }}>
                ...
                {proposal.length > 0 && (
                  <ImpactSection title={t('MODAL_EDIT_USER.IDS_TEXT_PROPOSE')} border="1px solid #e0e7ff" marginBottom={0}>
                    {proposal.map((line, i) => {
                      const isCaseHeader =
                      ...
                      return (
                        <div key={i} style={{
                          fontSize: FONT_SIZE, color: '#374151', lineHeight: 1.7, marginBottom: 2,
...
const Step1UserForm: React.FC<Step1UserFormProps> = React.memo(({
  isMultiUser, selectedRowKeys, companyList, mappingDivisionList, mappingDepartmentList,
  levelOptions, levelValue, isLoading, evaluationPeriod, timezone, handleDivisionChange, onChangeDepartment, t,
}) => (
  <div style={{ display: 'grid', gap: '15px' }}>
    {/* Evaluation period banner — single user only, shows active goal-setting window */}
    {!isLoading && evaluationPeriod.departmentGoal && evaluationPeriod.personalGoal && selectedRowKeys.length <= 1 && (
      <div style={{ backgroundColor: 'rgb(236 253 245)', padding: '0.5rem', borderRadius: '8px', borderLeft: '4px solid #047857' }}>
        ...
        <p style={{ color: '#047857', margin: 0, marginBottom: 0 }}>
      ...
      <Col span={12}>
        <Form.Item label={t('IDS_EVALUATION_SKILL')} name="flagSkill" colon={false} style={{ marginBottom: 0 }}>
          <ColoredSelect showSearch style={{ width: '100%' }} notFoundContent={<EmptyComponent />}>
            {isMultiUser && (
              <Select.Option value="-1" key="no-update">{t('IDS_NO_UPDATE')}</Select.Option>
            )}
            <Select.Option value={1} key={1}>{t('IDS_HAVE')}</Select.Option>
            <Select.Option value={0} key={0}>{t('IDS_NOT_HAVE')}</Select.Option>
          </ColoredSelect>
        </Form.Item>
      </Col>
    </Row>
...
const ModalEditUser: React.FC<ModalEditUserProps> = ({
  selectedRecords, setIsModalOpen, isModalOpen,
  selectedRowKeys, setSelectedRowKeys, setSelectedRows, handleSearch,
}) => {
  ...
  const levelOptions = useMemo(
    () => listLevels.map((item) => ({ value: item.id, label: item.level })),
    [listLevels],
  );
  ...
  const hasPersonalGoalLevel = useMemo(
    () => selectedRecords.some((r: any) => Number(r.level) < 8),
    [selectedRecords],
  );
  ...
  const { displayRadioOne, displayRadioTwo } = useMemo(() => {
    ...
    isMultiUser, selectedRecords, levelValue, companyValue,
    departmentValue, divisionValue, flagSkillValue, typeEvaluation, t,
  ]);
  ...
  const isNextDisabled = useMemo(() => {
    ...
    currentStep, isMultiUser, selectedRecords, companyValue, departmentValue,
    divisionValue, levelValue, flagSkillValue, radioLevelValue, typeEvaluation, displayRadioOne, displayRadioTwo, t,
  ]);
    ...
    Promise.all([
      ...
      .then(([companyRes, divisionRes, periodRes]) => {
        ...
        if (divisionRes?.status === 200) {
          ...
          if (isMultiUser) {
            setListDivisions([{ divisionId: t('IDS_NO_UPDATE'), codeName: t('IDS_NO_UPDATE'), childrens: [] }, ...dataList]);
        ...
        if (isMultiUser) {
          const noUpdateLabel = t('IDS_NO_UPDATE');
          form.setFieldsValue({ company: noUpdateLabel, division: noUpdateLabel, department: noUpdateLabel, level: noUpdateLabel, flagSkill: noUpdateLabel });
        } else {
          const record = selectedRecords[0] as any;
          form.setFieldsValue({
          ...
          if (record?.id) {
            httpAxios.Get('/api/v1/f8/management-user/get-evaluation-by-user', { params: { id: record.id } })
              .then((res) => {
                if (res?.status === 200) {
                  if (res.data?.length > 0) {
                    const first = res.data[0];
                    const inPeriod = compareDatePeriod(
                      first.level <= 7 ? first.evaluationPeriod.dateCreationGoalStart : first.evaluationPeriod.dateCreationGoalDepartmentStart,
                      first.level <= 7 ? first.evaluationPeriod.dateCreationGoalEnd : first.evaluationPeriod.dateCreationGoalDepartmentEnd,
                    );
  ...
  const gotoStep = useCallback(() => {
    form.validateFields(getValidateFields()).then(async () => {
  ...
  }, [form, selectedRecords, selectedRowKeys, radioLevelValue, t, setSelectedRowKeys, setSelectedRows, setIsModalOpen, handleSearch]);
  ...
  return (
    <Modal
      ...
      footer={null}
      closable={false}
      width={currentStep === 3 ? (isMultiUser ? 1100 : 800) : 600}
      ...
      >
        ...
        <div className={styles.headerSection}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '15px' }}>
            ...
            <span style={{ cursor: 'pointer', color: '#d1d5db' }} onClick={() => setIsModalOpen(false)}>✕</span>
          </div>
          ...
          <div className={styles.stepNavigation}>
            {stepsConfigs.map((s, index) => (
              <React.Fragment key={s.value}>
                ...
                {index < stepsConfigs.length - 1 && (
                  <RightOutlined style={{ fontSize: FONT_SIZE, color: '#000' }} />
                )}
              </React.Fragment>
            ))}
          </div>
        ...
        {/* Body */}
        <div style={{
          padding: currentStep === 3 ? '0' : '0 15px',
          flex: 1,
          ...
          {/* Step 2 */}
          {currentStep === 2 && (
            <Form.Item name="radioCheck" colon={false} valuePropName="checked" style={{ padding: '15px 0', marginBottom: 0 }}>
              <Radio.Group
        ...
        <div className={styles.footer}>
          <div style={{ display: 'flex', gap: FONT_SIZE }}>
            {currentStep < 3 ? (
