...
const ColumnBasic = (props: Props) => {
  ...
  useEffect(() => {
    const arr: any = [];
    props.dataSource?.settingPointBasic?.forEach((e: any) => {
      const countDuplicate = props.dataSource?.settingPointBasic?.filter((num) => e.point === num.point).length || 0;
  ...
  return [
            ...
            rules={[
              {
                validator: async (_, value) => {
                  if (value === '') return Promise.reject(new Error(t('MESSAGE.COMMON.IDM_BLANK_ITEM').toString()));
                    ...
                    const listSettingPointBasic = props.dataSource.settingPointBasic || [];
    ...
    },
    {
      ...
      render: (_text: any, record: any) => {
        ...
        return (
          props.dataSource.settingPointBasic!.length > MIN_ROW && (
            <Button
              icon={<DeleteOutlined />}
              style={{ color: '#007240 ' }}
              onClick={async () => {
                const dataList = props.dataSource.settingPointBasic!.filter((item: any) => item.key !== record.key);
                startTransition(() => props.setDataSource({ ...props.dataSource, settingPointBasic: [...dataList] }));
              }}
