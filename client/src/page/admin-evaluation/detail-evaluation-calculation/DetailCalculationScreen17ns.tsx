...
import DetailFormNs from '../../../views/admin-evaluation/detail-evaluation-calculation/components/DetailFormNs';
import { BEHAVIOR_KEY } from '../../../views/admin-evaluation/detail-evaluation-calculation/components/ItemTab';
import { VersionSettingStatus } from '../../../constant/VersionSettingStatus';
import { VersionSettingType } from '../../../constant/VersionSettingType';
import { onValidateSavePublicNs } from '../../../views/admin-evaluation/detail-evaluation-calculation/components/DetailCalculation17HandlerNs';
import { urlCompanyCode } from '../../../common/util';

const DetailCalculationScreen17ns = () => {
  ...
  const [currentTab, setCurrentTab] = useState(BEHAVIOR_KEY);
  const [form] = formState || Form.useForm();
  ...
  const onEdit = async () => {
    ...
    const reason = dataSource.status !== 1 ? '' : dataSource.reason;
    setDataSource({
      ...dataSource,
      reason,
  ...
  useEffect(() => {
    if (!record) {
      ...
      setLoadingPage(true);
      evaluationCalculationApiService.getDetailEvaluationCalculationNs(
        record?.id,
        callbackDetail,
        callBackNotFound,
        setLoading,
  ...
  const onPublicVersion = () => {
    ...
    if (subVersion === '0') {
      const payload: PublicEvaluationCalculationDto = {
        versionId: record.id,
        type: VersionSettingType.LEVEL_1_7_NO_SKILL,
  ...
  const onSaveDraft = () => {
    ...
    if (dataSource.status === VersionSettingStatus.PUBLIC) {
      ...
      evaluationCalculationApiService.saveDraftVersionSetting17ns(
        payload,
  ...
  const onSavePublicVersion = () => {
    ...
    evaluationCalculationApiService.savePublicVersionSetting17ns(
      payload,
      callbackSavePublic,
  ...
  const onCancelVersion = async () => {
    if (dataSource.status === 1) {
      const payload = { status: 2, updatedTime: dataSource.updatedTime };
...
export default DetailCalculationScreen17ns;
