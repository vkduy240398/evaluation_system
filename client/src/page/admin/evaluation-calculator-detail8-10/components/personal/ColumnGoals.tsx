...
import { SettingAchievementPersonalDto } from '../../../../../model/evaluation-calculation/DetailEvaluationCalculation810Model';
...
const ColumnGoalDiffs = (props: Props) => {
  ...
  useEffect(() => {
    const arr: any = [];
    props.dataHandling?.settingAchievementPersonalDiff?.forEach((e: any, _index: any) => {
      const countDuplicate =
        props.dataHandling?.settingAchievementPersonalDiff?.filter(
          (num: any) => e.point === num.point && num.point !== 0,
        ).length || 0;
            ...
            rules={[
              {
                validator: async (_, value) => {
                  if (value === '') return Promise.reject(new Error(t('MESSAGE.COMMON.IDM_BLANK_ITEM').toString()));
                    ...
                    const listSettingPersonalDiff = props.dataHandling.settingAchievementPersonalDiff || [];
          ...
          >
            <Input
              ...
              onChange={(e) => {
                ...
                const dataTemps = [...(props.dataHandling.settingAchievementPersonalDiff || [])];
                dataTemps[index] = { ...dataTemps[index], point: e.target.value || undefined };
                props.setDataHandling({ ...props.dataHandling, settingAchievementPersonalDiff: dataTemps });
        ...
        return !props.isEdit ? (
          ...
          >
            <TextArea
              ...
              onChange={(e) => {
                // const dataTemps = [...(props.dataSource.settingAchievementPersonalDiff || [])];
                // dataTemps[index] = { ...dataTemps[index], note: e.target.value || undefined };
                // startTransition(() =>
                //   props.setDataSource({ ...props.dataSource, settingAchievementPersonalDiff: dataTemps }),
                // );
          ...
          props.dataSource.settingAchievementPersonalDiff!.length > 1 && (
            <Button
              ...
              onClick={async () => {
                ...
                const dataListTab = props.dataHandling.settingAchievementPersonalDiff!.filter(
                  (item: any) => item.key !== record.key,
                );
                props.setDataHandling({ ...props.dataHandling, settingAchievementPersonalDiff: dataListTab });
              }}
...
const ColumnGoalJudgeIndexes = (props: Props) => {
  ...
  useEffect(() => {
    const arr: any = [];
    props.dataHandling?.settingAchievementPersonalJudgeIndex?.forEach((e: any, _index: any) => {
      const countDuplicate =
        props.dataHandling?.settingAchievementPersonalJudgeIndex?.filter(
          (num: any) => e.point === num.point && num.point !== 0,
        ).length || 0;
      if (countDuplicate > 1 || e.point > 0) {
        arr.push(`input_point_goal_${e.key}`);
                ...
                validator: async (_, value) => {
                  if (value === '') return Promise.reject(new Error(t('MESSAGE.COMMON.IDM_BLANK_ITEM').toString()));
                    ...
                    const listSettingAchievementPersonalJudgeIndex =
                      props.dataHandling.settingAchievementPersonalJudgeIndex || [];
          ...
          >
            <Input
              ...
              onChange={(e) => {
                ...
                // dataTemps[index] = { ...dataTemps[index], point: e.target.value };
                // startTransition(() =>
                //   props.setDataSource({ ...props.dataSource, settingAchievementPersonalJudgeIndex: dataTemps }),
                // );

                const dataTemps = [...(props.dataHandling.settingAchievementPersonalJudgeIndex || [])];
                dataTemps[index] = { ...dataTemps[index], point: e.target.value };
                props.setDataHandling({ ...props.dataHandling, settingAchievementPersonalJudgeIndex: dataTemps });
                isValueChange.current = true;
              }}
            />
          </Form.Item>
          ...
          props.dataSource.settingAchievementPersonalJudgeIndex!.length > 1 && (
            <Button
              ...
              onClick={async () => {
                ...
                const dataListTab = props.dataHandling.settingAchievementPersonalJudgeIndex!.filter(
                  (item: any) => item.key !== record.key,
                );
                props.setDataHandling({ ...props.dataHandling, settingAchievementPersonalJudgeIndex: dataListTab });
              }}
              loading={props.isLoading}
            ></Button>
          )
