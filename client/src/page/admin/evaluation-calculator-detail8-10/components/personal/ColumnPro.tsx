...
const checkDuplicatePoint = (value: number, indexException: number, listPoint: any[]) => {
  return listPoint.filter((el) => el !== listPoint.at(indexException)).some((el) => Number(el.point) === value);
};
...
//         props.dataTabProSkill?.settingProFormula?.filter((num: any) => e.point === num.point && num.point !== 0)
//           .length || 0;
...
//                   const listSettingProFormuala = props.dataTabProSkill.settingProFormula || [];
...
//               onClick={async () => {
//                 const dataListDataTab = props.dataTabProSkill.settingProFormula!.filter(
//                   (item: any) => item.key !== record.key,
//                 );
//                 startTransition(() => {
//                   props.setDataSource({ ...props.dataSource, settingProFormula: [...dataListDataTab] });
//                 });
//                 props.setDataTabProSkill({
//                   ...props.dataTabProSkill,
//                   settingProFormula: [...dataListDataTab],
//                 });
//               }}
...
export const ColumnPointPro = (props: Props) => {
  ...
  useEffect(() => {
    const arr: any = [];
    props.dataTabProSkill?.settingPointPro?.forEach((e: any, _index: any) => {
      const countDuplicate =
        props.dataTabProSkill?.settingPointPro?.filter((num: any) => e.point === num.point).length || 0;
      if (countDuplicate > 1 || e.point >= 0) {
  ...
  return [
        ...
        return !props.isEdit ? (
            ...
            rules={[
              {
                validator: async (_, value) => {
                  if (value === '') return Promise.reject(new Error(t('MESSAGE.COMMON.IDM_BLANK_ITEM').toString()));
                    ...
                    const listSettingPointPro = props.dataTabProSkill.settingPointPro || [];
          ...
          >
            <Input
              ...
              onChange={(e) => {
                const value = e.target.value;
                const dataTemp = [...(props.dataTabProSkill.settingPointPro || [])];
                dataTemp[index] = { ...dataTemp[index], point: value ? Number(value) : undefined };
                props.setDataTabProSkill({ ...props.dataTabProSkill, settingPointPro: dataTemp });
                isValueChange.current = true;
              }}
            />
          </Form.Item>
    ...
    },
    {
      ...
      render: (_text: any, record: any) => {
        return (
          props.dataTabProSkill.settingPointPro!.length > 1 && (
            <Button
              icon={<DeleteOutlined style={{ color: '#007240 ' }} />}
              onClick={async () => {
                const dataListTab = props.dataTabProSkill.settingPointPro!.filter(
                  (item: any) => item.key !== record.key,
                );
                startTransition(() => {
