...
export default function ShowMoreMailPopUp(props: Props) {
  ...
  const adminMail = 'gnw-legal@geonet.co.jp';
  ...
  const handleSaveNew = async () => {
    if (selectedMailList.length) {
      ...
      if (wrongFormatList.length) {
        ...
        tempMailList.unshift(adminMail);
        ...
        displayMailList.unshift({ value: adminMail, label: adminMail, key: adminMail });
  ...
  const rowSelection = {
    ...
    getCheckboxProps: (record: any) => {
      return { disabled: record.value == 'gnw-legal@geonet.co.jp' || isDisabled, name: record.value };
    },
  };
  const searchField = () => {
    return (
