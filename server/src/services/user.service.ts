...
export class UserService {
  ...
  async getEvaluationByUserId(id: any, companyGroupCode: string, timeZone: string) {
  ...
  async importUserProcedue(
    ...
    if (period?.id) {
      const importedUserIds = await this.userRepo.getEvaluatorDefaultUserIds(
        period.id,
        companyGroupCode,
      );
      ...
      // Step 3: Override names to reflect the configured setting hierarchy.
