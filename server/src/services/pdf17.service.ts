/* eslint-disable no-await-in-loop */
/* eslint-disable @typescript-eslint/naming-convention */
/* eslint-disable @typescript-eslint/no-var-requires */
import autoTable from 'jspdf-autotable';
import { EvaluationHelper } from 'src/common/EvaluationHelper';
import { Pdf17Helper } from 'src/common/pdf/Pdf17Helper';
import { EvaluationPeriodHelper } from 'src/common/datetime/EvaluationPeriodHelper';
import { Evaluation } from 'src/entity/Evaluation';
import { PdfServiceI } from 'src/interfaces/service/pdf.service.interface';
import { ReportResponseDto } from 'src/model/response/ReportResponseDto';
import { Pdf17Helper2 } from 'src/common/pdf/Pdf17Helper2';
import { jsPDF } from 'jspdf';
import msGothic from 'src/static/fonts/MS-Gothic/MsGothic';

const LEFT_BRACKET = '【';
const RIGHT_BRACKET = '】';
const YEAR = '年';
const SUFFIX = '評価表.pdf';
const MIME_LOGO = 'png';
const BASE64_IMAGE =
  '';

export class Pdf17Service implements PdfServiceI {
  private initDocument() {
    const doc = new jsPDF();

    const mainFont = msGothic;
    doc.addFileToVFS('MS-Gothic.ttf', mainFont);
    doc.addFont('MS-Gothic.ttf', 'msgothic-normal', 'normal');
    doc.setFont('msgothic-normal');

    return doc;
  }

  private getPageHeight(doc: jsPDF) {
    return doc.internal.pageSize.height || doc.internal.pageSize.getHeight();
  }

  private getPageWidth(doc: jsPDF) {
    return doc.internal.pageSize.width || doc.internal.pageSize.getWidth();
  }

  /**
   * Generate report with choosen template level 1~7
   *
   * @author tran.le.ha.nam
   * @last_update
   * @param evaluation - evaluation object
   * @param type - template type
   */
  async exportEvaluationReportPdf(evaluation: Evaluation, type: number) {
    const fileName = this.getFileName(evaluation.user.fullName);

    let doc = this.initDocument();

    if (type === 1) {
      doc = this.getTemplateOneReportFrame(evaluation, doc);
    }
    if (type === 2) {
      doc = this.getTemplateTwoReportFrame(evaluation, doc);
    }
    if (type === 3) {
      doc = this.getTemplateThreeReportFrame(evaluation, doc);
    }

    const arrayBuffer = await doc.output('arraybuffer');

    const results = new ReportResponseDto();
    results.filename = fileName;
    results.buffer = Buffer.from(arrayBuffer);

    return results;
  }

  private getTemplateOneReportFrame(evaluation: Evaluation, doc: jsPDF) {
    const pageHeight = this.getPageHeight(doc);
    const pageWidth = this.getPageWidth(doc);
    let currenY = 0;

    const exist05 = evaluation.evaluator.some((el) => {
      return el.evaluationOrder.toString() === '0.5';
    })
      ? true
      : false;

    const exist1 = evaluation.evaluator.some((el) => {
      return el.evaluationOrder.toString() === '1.0';
    })
      ? true
      : false;

    const exist2 = evaluation.evaluator.some((el) => {
      return el.evaluationOrder.toString() === '2.0';
    })
      ? true
      : false;

    const logoData = new Buffer(BASE64_IMAGE, 'base64');
    doc.addImage(logoData, MIME_LOGO, 5, 5, 30, 12.5);

    doc.setFontSize(20);
    doc.text('EVALUATION REPORT', pageWidth / 2, 30, {
      align: 'center',
    });
    currenY += 40;

    doc.setFontSize(11);
    doc.text(`従業員名: ${evaluation.user.fullName}`, 5, currenY);
    doc.text(
      `評価学期: ${evaluation.periodStart} ～ ${evaluation.periodEnd}`,
      pageWidth - pageWidth / 4,
      40,
      {
        align: 'left',
      },
    );
    currenY += 5;

    doc.text(`従業員番号: ${evaluation.user.employeeNumber}`, 5, currenY);
    doc.text(
      `給与ランク: ${evaluation.level}`,
      pageWidth - pageWidth / 4,
      currenY,
      {
        align: 'left',
      },
    );
    currenY += 5;

    doc.text(`会社: ${evaluation.companyName}`, 5, currenY);
    currenY += 5;

    doc.text(`部署: ${evaluation.departmentName}`, 5, currenY);
    currenY += 5;

    doc.text(
      `評価者: ${EvaluationHelper.getOrderEvaluators(evaluation.evaluator)}`,
      5,
      currenY,
    );
    currenY += 5;

    // Table summary
    autoTable(doc, {
      styles: { font: 'msgothic-normal' },
      theme: 'grid',
      startY: currenY,
      head: [Pdf17Helper.getHeaderSummaryTable()],
      body: Pdf17Helper.getSummaryData(evaluation),
    });
    currenY = (doc as any).lastAutoTable.finalY + 10;

    // Table basic
    doc.text(`基本的な技能・技術に関する評価`, 5, currenY);
    currenY += 2;
    autoTable(doc, {
      theme: 'grid',
      styles: { font: 'msgothic-normal' },
      margin: { left: 5, right: 5 },
      startY: currenY,
      head: [Pdf17Helper.getHeaderBasicTable(exist05)],
      columnStyles: { 0: { cellWidth: 35 }, 1: { cellWidth: 100 } },
      body: Pdf17Helper.getBasicTableData(evaluation, 1, exist05),
    });
    currenY = (doc as any).lastAutoTable.finalY + 10;

    // Table pro
    doc.text(`専門的事項`, 5, currenY);
    currenY += 2;
    autoTable(doc, {
      theme: 'grid',
      styles: { font: 'msgothic-normal' },
      margin: { left: 5, right: 5 },
      startY: currenY,
      head: [Pdf17Helper.getHeaderBasicTable(exist05)],
      columnStyles: { 0: { cellWidth: 35 }, 1: { cellWidth: 100 } },
      body: Pdf17Helper.getProTableData(evaluation, exist05),
    });
    currenY = (doc as any).lastAutoTable.finalY + 10;

    // Table behavior
    doc.text(`等級別行動`, 5, currenY);
    currenY += 2;
    autoTable(doc, {
      theme: 'grid',
      styles: { font: 'msgothic-normal' },
      margin: { left: 5, right: 5 },
      startY: currenY,
      head: [Pdf17Helper.getHeaderBasicTable(exist05)],
      columnStyles: { 0: { cellWidth: 35 }, 1: { cellWidth: 100 } },
      body: Pdf17Helper.getBasicTableData(evaluation, 2, exist05),
    });
    currenY = (doc as any).lastAutoTable.finalY + 10;

    // Table personal goal
    doc.text(`個人目標`, 5, currenY);
    currenY += 2;
    autoTable(doc, {
      theme: 'grid',
      styles: { font: 'msgothic-normal' },
      margin: { left: 5, right: 5 },
      startY: currenY,
      head: [Pdf17Helper.getHeaderGoalTable(exist05)],
      columnStyles: { 0: { cellWidth: 35 }, 1: { cellWidth: 100 } },
      body: Pdf17Helper.getGoalTableData(evaluation, exist05),
    });
    currenY = (doc as any).lastAutoTable.finalY;
    autoTable(doc, {
      theme: 'grid',
      styles: { font: 'msgothic-normal' },
      margin: { left: 5, right: 5 },
      startY: currenY,
      columnStyles: Pdf17Helper.getColumnTypeTotalPointGoalTable(exist05),
      body: Pdf17Helper.getTotalPointGoalTableData(evaluation, exist05),
    });
    currenY = (doc as any).lastAutoTable.finalY + 10;

    // Table achievement
    doc.text(`個人成果`, 5, currenY);
    currenY += 2;
    autoTable(doc, {
      theme: 'grid',
      styles: { font: 'msgothic-normal' },
      margin: { left: 5, right: 5 },
      startY: currenY,
      head: [Pdf17Helper.getHeaderAchievementTable(exist05)],
      columnStyles: {
        0: { cellWidth: 35 },
        1: { cellWidth: 50 },
        2: { cellWidth: 50 },
      },
      body: Pdf17Helper.getAchievementTableData(evaluation, exist05),
    });
    currenY = (doc as any).lastAutoTable.finalY;
    autoTable(doc, {
      theme: 'grid',
      styles: { font: 'msgothic-normal' },
      margin: { left: 5, right: 5 },
      startY: currenY,
      columnStyles:
        Pdf17Helper.getColumnTypeTotalPointAchievementTable(exist05),
      body: Pdf17Helper.getTotalPointAchievementTableData(evaluation, exist05),
    });
    currenY = (doc as any).lastAutoTable.finalY + 10;

    // Table additional
    doc.text(`期中発生した追加目標および事項（考慮項目）`, 5, currenY);
    currenY += 2;
    autoTable(doc, {
      theme: 'grid',
      styles: { font: 'msgothic-normal' },
      margin: { left: 5, right: 5 },
      startY: currenY,
      head: [Pdf17Helper.getHeaderAdditionalTable(exist05)],
      columnStyles: {
        0: { cellWidth: 35 },
        1: { cellWidth: 50 },
        2: { cellWidth: 50 },
      },
      body: Pdf17Helper.getAdditionalTableData(evaluation, exist05),
    });
    currenY = (doc as any).lastAutoTable.finalY;
    autoTable(doc, {
      theme: 'grid',
      styles: { font: 'msgothic-normal' },
      margin: { left: 5, right: 5 },
      startY: currenY,
      columnStyles: Pdf17Helper.getColumnTypeTotalPointAdditionalTable(exist05),
      body: Pdf17Helper.getTotalPointAdditionalTableData(evaluation, exist05),
    });
    currenY = (doc as any).lastAutoTable.finalY + 10;

    const rater05 = Pdf17Helper.getEvaluatorByOrder(
      evaluation.evaluator,
      '0.5',
    );
    const rater1 = Pdf17Helper.getEvaluatorByOrder(evaluation.evaluator, '1.0');
    const rater2 = Pdf17Helper.getEvaluatorByOrder(evaluation.evaluator, '2.0');

    // User comment
    doc.text(`自己評価`, 5, currenY);
    currenY += 1;
    const commentUser = doc.splitTextToSize(evaluation.commentUser, 195);
    const CommentUserDimension = doc.getTextDimensions(commentUser);
    if (CommentUserDimension.h + currenY > pageHeight) {
      doc.addPage();
      currenY = 5;
    }
    doc.rect(5, currenY, pageWidth - 10, CommentUserDimension.h + 5);
    currenY += 5;
    doc.text(commentUser, 8, currenY);
    currenY += CommentUserDimension.h + 10;

    // Rater 0.5 comment
    if (exist05) {
      doc.text(`0.5次評価`, 5, currenY);
      currenY += 1;
      const commentRater05 = doc.splitTextToSize(rater05.commentPublic, 195);
      const CommentRater05Dimension = doc.getTextDimensions(commentRater05);
      if (CommentRater05Dimension.h + currenY > pageHeight) {
        doc.addPage();
        currenY = 5;
      }
      doc.rect(5, currenY, pageWidth - 10, CommentRater05Dimension.h + 5);
      currenY += 5;
      doc.text(commentRater05, 8, currenY);
      currenY += CommentRater05Dimension.h + 10;
    }

    // Rater 1 comment
    if (exist1) {
      doc.text(`一次評価`, 5, currenY);
      currenY += 1;
      const commentRater1 = doc.splitTextToSize(rater1.commentPublic, 195);
      const CommentRater1Dimension = doc.getTextDimensions(commentRater1);
      if (CommentRater1Dimension.h + currenY > pageHeight) {
        doc.addPage();
        currenY = 5;
      }
      doc.rect(5, currenY, pageWidth - 10, CommentRater1Dimension.h + 5);
      currenY += 5;
      doc.text(commentRater1, 8, currenY);
      currenY += CommentRater1Dimension.h + 10;
    }

    if (exist2) {
      // Rater 2 comment
      doc.text(`二次評価`, 5, currenY);
      currenY += 1;
      const commentRater2 = doc.splitTextToSize(rater2.commentPublic, 195);
      const CommentRater2Dimension = doc.getTextDimensions(commentRater2);
      if (CommentRater2Dimension.h + currenY > pageHeight) {
        doc.addPage();
        currenY = 5;
      }
      doc.rect(5, currenY, pageWidth - 10, CommentRater2Dimension.h + 5);
      currenY += 5;
      doc.text(commentRater2, 8, currenY);
      currenY += CommentRater2Dimension.h + 10;
    }

    return doc;
  }

  private getTemplateTwoReportFrame(evaluation: Evaluation, doc: jsPDF) {
    const pageWidth = this.getPageWidth(doc);
    let currenY = 0;

    const logoData = new Buffer(BASE64_IMAGE, 'base64');

    doc.addImage(logoData, MIME_LOGO, 5, 5, 30, 12.5);

    doc.setFontSize(20);
    doc.text('EVALUATION REPORT', pageWidth / 2, 30, {
      align: 'center',
    });
    currenY += 40;

    doc.setFontSize(11);
    doc.text(`従業員名: ${evaluation.user.fullName}`, 5, currenY);
    doc.text(
      `評価学期: ${evaluation.periodStart} ～ ${evaluation.periodEnd}`,
      pageWidth - pageWidth / 4,
      40,
      {
        align: 'left',
      },
    );
    currenY += 5;

    doc.text(`従業員番号: ${evaluation.user.employeeNumber}`, 5, currenY);
    doc.text(
      `給与ランク: ${evaluation.level}`,
      pageWidth - pageWidth / 4,
      currenY,
      {
        align: 'left',
      },
    );
    currenY += 5;

    doc.text(`会社: ${evaluation.companyName}`, 5, currenY);
    currenY += 5;

    doc.text(`部署: ${evaluation.departmentName}`, 5, currenY);
    currenY += 5;

    doc.text(
      `評価者: ${EvaluationHelper.getOrderEvaluators(evaluation.evaluator)}`,
      5,
      currenY,
    );
    currenY += 5;

    // Table summary
    autoTable(doc, {
      styles: { font: 'msgothic-normal' },
      theme: 'grid',
      startY: currenY,
      head: [Pdf17Helper2.getHeaderSummaryTable()],
      body: Pdf17Helper2.getSummaryData(evaluation),
    });
    currenY = (doc as any).lastAutoTable.finalY + 10;

    // Table basic
    doc.text(`基本的な技能・技術に関する評価`, 5, currenY);
    currenY += 2;
    autoTable(doc, {
      theme: 'grid',
      styles: { font: 'msgothic-normal' },
      margin: { left: 5, right: 5 },
      startY: currenY,
      head: [Pdf17Helper2.getHeaderBasicTable()],
      columnStyles: { 0: { cellWidth: 35 }, 1: { cellWidth: 100 } },
      body: Pdf17Helper2.getBasicTableData(evaluation, 1),
    });
    currenY = (doc as any).lastAutoTable.finalY + 10;

    // Table pro
    doc.text(`専門的事項`, 5, currenY);
    currenY += 2;
    autoTable(doc, {
      theme: 'grid',
      styles: { font: 'msgothic-normal' },
      margin: { left: 5, right: 5 },
      startY: currenY,
      head: [Pdf17Helper2.getHeaderBasicTable()],
      columnStyles: { 0: { cellWidth: 35 }, 1: { cellWidth: 100 } },
      body: Pdf17Helper2.getProTableData(evaluation),
    });
    currenY = (doc as any).lastAutoTable.finalY + 10;

    // Table behavior
    doc.text(`等級別行動`, 5, currenY);
    currenY += 2;
    autoTable(doc, {
      theme: 'grid',
      styles: { font: 'msgothic-normal' },
      margin: { left: 5, right: 5 },
      startY: currenY,
      head: [Pdf17Helper2.getHeaderBasicTable()],
      columnStyles: { 0: { cellWidth: 35 }, 1: { cellWidth: 100 } },
      body: Pdf17Helper2.getBasicTableData(evaluation, 2),
    });
    currenY = (doc as any).lastAutoTable.finalY + 10;

    // Table personal goal
    doc.text(`個人目標`, 5, currenY);
    currenY += 2;
    autoTable(doc, {
      theme: 'grid',
      styles: { font: 'msgothic-normal' },
      margin: { left: 5, right: 5 },
      startY: currenY,
      head: [Pdf17Helper2.getHeaderGoalTable()],
      columnStyles: { 0: { cellWidth: 35 }, 1: { cellWidth: 100 } },
      body: Pdf17Helper2.getGoalTableData(evaluation),
    });
    currenY = (doc as any).lastAutoTable.finalY + 10;

    // Table achievement
    doc.text(`個人成果`, 5, currenY);
    currenY += 2;
    autoTable(doc, {
      theme: 'grid',
      styles: { font: 'msgothic-normal' },
      margin: { left: 5, right: 5 },
      startY: currenY,
      head: [Pdf17Helper2.getHeaderAchievementTable()],
      columnStyles: {
        0: { cellWidth: 35 },
        1: { cellWidth: 50 },
        2: { cellWidth: 50 },
      },
      body: Pdf17Helper2.getAchievementTableData(evaluation),
    });
    currenY = (doc as any).lastAutoTable.finalY + 10;

    return doc;
  }

  private getTemplateThreeReportFrame(evaluation: Evaluation, doc: jsPDF) {
    const pageHeight = this.getPageHeight(doc);
    const pageWidth = this.getPageWidth(doc);
    let currenY = 0;

    const logoData = new Buffer(BASE64_IMAGE, 'base64');

    doc.addImage(logoData, MIME_LOGO, 5, 5, 30, 12.5);

    doc.setFontSize(20);
    doc.text('EVALUATION REPORT', pageWidth / 2, 30, {
      align: 'center',
    });
    currenY += 40;

    doc.setFontSize(11);
    doc.text(`従業員名: ${evaluation.user.fullName}`, 5, currenY);
    doc.text(
      `評価学期: ${evaluation.periodStart} ～ ${evaluation.periodEnd}`,
      pageWidth - pageWidth / 4,
      40,
      {
        align: 'left',
      },
    );
    currenY += 5;

    doc.text(`従業員番号: ${evaluation.user.employeeNumber}`, 5, currenY);
    doc.text(
      `給与ランク: ${evaluation.level}`,
      pageWidth - pageWidth / 4,
      currenY,
      {
        align: 'left',
      },
    );
    currenY += 5;

    doc.text(`会社: ${evaluation.companyName}`, 5, currenY);
    currenY += 5;

    doc.text(`部署: ${evaluation.departmentName}`, 5, currenY);
    currenY += 5;

    doc.text(
      `評価者: ${EvaluationHelper.getOrderEvaluators(evaluation.evaluator)}`,
      5,
      currenY,
    );
    currenY += 5;

    // Table summary
    autoTable(doc, {
      styles: { font: 'msgothic-normal' },
      theme: 'grid',
      startY: currenY,
      head: [Pdf17Helper2.getHeaderSummaryTable()],
      body: Pdf17Helper2.getSummaryData(evaluation),
    });
    currenY = (doc as any).lastAutoTable.finalY + 10;

    // Table basic
    doc.text(`基本的な技能・技術に関する評価`, 5, currenY);
    currenY += 2;
    autoTable(doc, {
      theme: 'grid',
      styles: { font: 'msgothic-normal' },
      margin: { left: 5, right: 5 },
      startY: currenY,
      head: [Pdf17Helper2.getHeaderBasicTable()],
      columnStyles: { 0: { cellWidth: 35 }, 1: { cellWidth: 100 } },
      body: Pdf17Helper2.getBasicTableData(evaluation, 1),
    });
    currenY = (doc as any).lastAutoTable.finalY + 10;

    // Table pro
    doc.text(`専門的事項`, 5, currenY);
    currenY += 2;
    autoTable(doc, {
      theme: 'grid',
      styles: { font: 'msgothic-normal' },
      margin: { left: 5, right: 5 },
      startY: currenY,
      head: [Pdf17Helper2.getHeaderBasicTable()],
      columnStyles: { 0: { cellWidth: 35 }, 1: { cellWidth: 100 } },
      body: Pdf17Helper2.getProTableData(evaluation),
    });
    currenY = (doc as any).lastAutoTable.finalY + 10;

    // Table behavior
    doc.text(`等級別行動`, 5, currenY);
    currenY += 2;
    autoTable(doc, {
      theme: 'grid',
      styles: { font: 'msgothic-normal' },
      margin: { left: 5, right: 5 },
      startY: currenY,
      head: [Pdf17Helper2.getHeaderBasicTable()],
      columnStyles: { 0: { cellWidth: 35 }, 1: { cellWidth: 100 } },
      body: Pdf17Helper2.getBasicTableData(evaluation, 2),
    });
    currenY = (doc as any).lastAutoTable.finalY + 10;

    // Table personal goal
    doc.text(`個人目標`, 5, currenY);
    currenY += 2;
    autoTable(doc, {
      theme: 'grid',
      styles: { font: 'msgothic-normal' },
      margin: { left: 5, right: 5 },
      startY: currenY,
      head: [Pdf17Helper2.getHeaderGoalTable()],
      columnStyles: { 0: { cellWidth: 35 }, 1: { cellWidth: 100 } },
      body: Pdf17Helper2.getGoalTableData(evaluation),
    });
    currenY = (doc as any).lastAutoTable.finalY + 10;

    // Table achievement
    doc.text(`個人成果`, 5, currenY);
    currenY += 2;
    autoTable(doc, {
      theme: 'grid',
      styles: { font: 'msgothic-normal' },
      margin: { left: 5, right: 5 },
      startY: currenY,
      head: [Pdf17Helper2.getHeaderAchievementTable()],
      columnStyles: {
        0: { cellWidth: 35 },
        1: { cellWidth: 50 },
        2: { cellWidth: 50 },
      },
      body: Pdf17Helper2.getAchievementTableData(evaluation),
    });
    currenY = (doc as any).lastAutoTable.finalY + 10;

    // User comment
    doc.text(`自己評価`, 5, currenY);
    currenY += 1;
    const commentUser = doc.splitTextToSize(evaluation.commentUser, 195);
    const CommentUserDimension = doc.getTextDimensions(commentUser);
    if (CommentUserDimension.h + currenY > pageHeight) {
      doc.addPage();
      currenY = 5;
    }
    doc.rect(5, currenY, pageWidth - 10, CommentUserDimension.h + 5);
    currenY += 5;
    doc.text(commentUser, 8, currenY);

    return doc;
  }

  private getSummaryPeriodTable(evaluations: Evaluation[], doc: jsPDF) {
    const pageWidth = this.getPageWidth(doc);
    let currenY = 0;

    const logoData = new Buffer(BASE64_IMAGE, 'base64');

    doc.addImage(logoData, MIME_LOGO, 5, 5, 30, 12.5);

    doc.setFontSize(20);
    doc.text('EVALUATION REPORT', pageWidth / 2, 30, {
      align: 'center',
    });
    currenY += 40;

    doc.setFontSize(11);
    doc.text(`従業員名: ${evaluations[0].user.fullName}`, 5, currenY);
    // doc.text(
    //   // `評価期間: ${EvaluationPeriodHelper.getCurrentPeriodYear()}${EvaluationPeriodHelper.getCurrentPeriodIndex()}期`,
    //   pageWidth - pageWidth / 4.8,
    //   40,
    //   {
    //     align: 'left',
    //   },
    // );
    currenY += 5;

    doc.text(`従業員番号: ${evaluations[0].user.employeeNumber}`, 5, currenY);
    doc.text(
      `給与ランク: ${evaluations[0].level}`,
      pageWidth - pageWidth / 4.8,
      currenY,
      {
        align: 'left',
      },
    );
    currenY += 5;

    doc.text(`会社: ${evaluations[0].companyName}`, 5, currenY);
    currenY += 5;

    doc.text(`部署: ${evaluations[0].departmentName}`, 5, currenY);
    currenY += 5;

    // Table summary
    autoTable(doc, {
      styles: { font: 'msgothic-normal' },
      theme: 'grid',
      startY: currenY,
      head: [Pdf17Helper.getHeaderSummaryPeriodTable()],
      // body: Pdf17Helper.getSummaryPeriodTableData(evaluations),
    });
    currenY = (doc as any).lastAutoTable.finalY + 10;

    return doc;
  }

  private getFileName(fullname: string) {
    const fileName = '';
    // LEFT_BRACKET +
    // EvaluationPeriodHelper.getCurrentPeriodYear() +
    // YEAR +
    // EvaluationPeriodHelper.getCurrentPeriodIndex() +
    // RIGHT_BRACKET +
    // fullname +
    // SUFFIX;

    return fileName;
  }

  /**
   * Return array buffer of child record
   *
   * @author tran.le.ha.nam
   * @last_update
   * @param evaluations - evaluation object
   * @param type - template type
   */
  private async getChildEvaluationReportPdf(
    doc: jsPDF,
    evaluation: Evaluation,
    type: number,
  ) {
    if (type === 1) {
      doc = await this.getTemplateOneReportFrame(evaluation, doc);
    }
    if (type === 2) {
      doc = await this.getTemplateTwoReportFrame(evaluation, doc);
    }
    if (type === 3) {
      doc = await this.getTemplateThreeReportFrame(evaluation, doc);
    }

    return doc;
  }

  /**
   * Merge child PDF docs
   *
   * @author tran.le.ha.nam
   * @last_update
   * @param evaluations - evaluation object
   * @param type - template type
   */
  async exportParentReportPdf(evaluations: Evaluation[]) {
    const templateOneStatus = 100;
    const templateTwoStatus = Array.from(new Array(50), (x, i) => i + 1);
    const templateThreeStatus = Array.from(new Array(49), (x, i) => i + 51);
    const fileName = this.getFileName(evaluations[0].user.fullName);

    let doc = this.initDocument();

    for (let i = 0; i < evaluations.length; i++) {
      if (i > 0) {
        doc.addPage();
      }

      if (templateOneStatus === evaluations[i].status) {
        doc = await this.getChildEvaluationReportPdf(doc, evaluations[i], 1);
      }

      if (templateTwoStatus.includes(evaluations[i].status)) {
        doc = await this.getChildEvaluationReportPdf(doc, evaluations[i], 2);
      }

      if (templateThreeStatus.includes(evaluations[i].status)) {
        doc = await this.getChildEvaluationReportPdf(doc, evaluations[i], 3);
      }
    }

    doc.addPage();
    doc = await this.getSummaryPeriodTable(evaluations, doc);

    const arrayBuffer = await doc.output('arraybuffer');

    const results = new ReportResponseDto();
    results.filename = fileName;
    results.buffer = Buffer.from(arrayBuffer);

    return results;
  }
}
