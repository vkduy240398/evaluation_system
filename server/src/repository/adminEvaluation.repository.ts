...
import { SummaryDepartment } from '../entity/SummaryDepartment';
...
@Injectable()
export class AdminEvaluationRepository implements AdminEvaluationRepositoryI {
  ...
  private settingAchievementPersonalRepository: typeof SettingAchievementPersonal;

  @Inject(EntityConstant.SETTING_ACHIEVEMENT_ADDITIONAL)
  private settingAchievementAdditionalRepository: typeof SettingAchievementAdditional;

  @Inject(EntityConstant.SETTING_FORMULA_8_10)
  private settingFormula810: typeof SettingFormula810;
  ...
  private evaluationPeriodRepository: typeof EvaluationPeriod;

  @Inject(EntityConstant.HISTORY_FIX_EVALUATION)
  private historyFixEvaluation: typeof HistoryFixEvaluation;
  ...
  async getListEvaluationConfirm(body: any, companyGroupCode: string) {
    const { periodId } = body;
  ...
  async goalConfirm(periodId: number, companyGroupCode: string): Promise<any> {
...
order by ${(() => {
        ...
        if (index >= 0) {
          sortStr = `"level" ${this.escapeSortDirection(
        ...
        if (index >= 0) {
          sortStr += `"status" ${this.escapeSortDirection(
            params.sortDirections[index],
          )},`;
        }
        return sortStr;
...
`,
    ...
    return { counts: evaluationList1[0]?.count || 0, data: evaluationList1 };
  }
  ...
  async exportCSV(params: any, companyGroupCode: string): Promise<any> {
    const { email, department, salaryRank, title, status } = params;

    const departmentName =
      // department === 'すべて' ? '' : `${department[1]}:${department[2]}`;
      department === 'すべて' ? '' : `${department[0]}`;
    ...
    const datas: any = await this.evaluationRepository.sequelize.query(
      `
...
inner join (SELECT j.key::int, j.value from json_each_text('${JSON.stringify(
        statusEvaluation,
      )}') as j)
as STT on TMP.status=STT.key inner join evaluation_period_tbl P on TMP.evaluation_period_id=P.id where (TMP.arr_status::int[] && array[${
        arrOfStatus.toString() || -1
      }]) 
...
${
  departmentName &&
  `and '${departmentName}' = ${
    department[1]?.trim() === '0' ? 'ANY(arr_department)' : 'division'
  } `
}
...
order by TMP.employee_number ASC 
  ...
  async publicEvaluation(body: any, companyGroupCode: string): Promise<any> {
    const { periodId } = body;
    const t = await this.evaluationRepository.sequelize.transaction();
    try {
      await this.evaluationRepository.update(
  ...
  async getAchievementPersonal(versionId: number) {
    return await this.settingAchievementPersonalRepository.findAll({
      ...
      //   '"SettingAchievementPersonal"."point" ASC NULLS LAST ',
      // ),
      order: [['point', 'ASC']],
    });
  }

  async getAchievementDep(
    versionId: number,
    type: 1 | 2,
  ...
  async getAchievementAdditional(versionId: number, type: TypeAchievement) {
    return await this.settingAchievementAdditionalRepository.findAll({
      where: { versionId: versionId, type: type },
      order: Sequelize.literal(
        '"SettingAchievementAdditional"."point" DESC NULLS LAST ',
      ),
      // order: [['point', 'DESC']],
    });
  }

  async getAchievementAdditionalDep(versionId: number) {
    return await this.settingAchievementAdditionalRepository.findAll({
  ...
  async getFormula(versionId: number) {
    return await this.settingFormula810.findAll({
      where: { versionId: versionId },
      order: Sequelize.literal('"SettingFormula810"."point" DESC NULLS LAST '),
      // order: [['point', 'DESC']],
    });
  }

  async getData810(versionId: number) {
  ...
  async haveRecordEdit(req: Request) {
  ...
  async haveRecordEditNS(req: Request) {
    const editRecord = await this.versionSettingEntity.findOne({
      where: {
        [Op.and]: [
          { status: 1 },
  ...
  // async listUserEvaluationPeriod(params: any) {
  //   const { periodId, stringStatus, type } = params;
  //   const arrStatus = stringStatus
  //     ? stringStatus.split(',').map((num: any) => parseFloat(num))
  //     : [];
  //   const datas = await this.evaluationRepository.findAll({
  //     where: {
  //       [Op.and]: [
  //         { evaluationPeriodId: periodId },
  //         { status: { [Op.in]: arrStatus } },
  //         {
  //           status: {
  //             [Op.lt]:
  //               type === 'fixedGoal'
  ...
  //         model: User,
  //         as: 'user',
  //         attributes: ['fullName', 'employeeNumber', 'email'],
  //         include: [
  //           {
  //             model: EvaluatorDefault,
  //             as: 'evaluatorDefaults',
  //             attributes: ['evaluationPeriodId'],
  //           },
  //         ],
  //       },
  //       {
  ...
  //     order: [
  //       // ['status', 'ASC'],
  //       [{ model: User, as: 'user' }, 'employeeNumber', 'ASC'],
  //     ],
  //   });
...
order by
	ut.employee_number asc`,
      {
        replacements: {
          evaluationStatus: JSON.stringify(statusEvaluation),
          timeZone: 'Asia/Tokyo',
          periodId: periodId,
          companyGroupCode: companyGroupCode,
          type: type,
  ...
  async getEvaluatinPeriod(type: string, timeZone: string) {
    const today = moment().tz(timeZone).format('YYYY/MM');
    const evaluationPeriods = await this.evaluationPeriodRepository.findAll({
      order: [['id', 'ASC']],
    });
    let data: any = {};
    evaluationPeriods.forEach((e: any) => {
      if (
        moment(e.periodStart).tz(timeZone).format('YYYY/MM') <= today &&
  ...
  async getAllEvaluation(condition: any): Promise<any> {
    return await this.evaluationRepository.findAll({
      where: condition,
  ...
  async evaluationFixed(yearStart: any, yearEnd: any): Promise<any> {
  ...
  async getAllSkillByCondition(userId: number): Promise<any> {
  ...
  async countAllEvaluationFixed(query: any): Promise<any> {
  ...
  async countEvaluationFixed(
    type: string,
    periodId: number,
    companyGroupCode: string,
  ): Promise<any> {
    // const query = `select count(*) from evaluation_tbl,evaluation_period_tbl where evaluation_tbl.evaluation_period_id= evaluation_period_tbl.id and evaluation_period_tbl.id=${periodId} and status<${
    //   type === 'goal' ? '49' : type === 'evaluation' ? '98' : '99'
    // }`;
...
where evaluation_period_tbl.id = :periodId
  ...
  async totalEvaluation(
    ...
    // const datas = await this.evaluationRepository.count({
    //   where: {
    //     [Op.and]: [
    ...
    //       model: EvaluationPeriod,
    //       // attributes: [],
    //       include: [{ model: EvaluatorDefault, as: 'evaluatorDefault' }],
    //       required: true,
    //     },
    //   ],
    //   logging: true,
...
on "evaluator_default_tbl".user_id = "Evaluation".user_id
  ...
  async findHistoryFixEvaluation(periodId: number, type: number) {
  ...
  async undoGoal(data: { [key: string]: string }, t: Transaction) {
    return await this.evaluationRepository.bulkCreate(
  ...
  async undoEvaluation(data: { [key: string]: string }, t: Transaction) {
    return await this.evaluationRepository.bulkCreate(
      Object.keys(data).map((id) => ({
        id: +id,
        status: +data[id],
  ...
  async deleteHistoryEvaluationFixed(periodId: number, t: Transaction) {
  ...
  async addProSkill(payload: AddProSkillDto, companyGroupCode: string) {
    ...
    try {
      ...
      const departments = [...payload.departments, ...payload.divisions].map(
        (dep) => ({
          skillId: newSkill.id,
          departmentId: dep,
        }),
      ...
      const skillData = await this.skillEntity.findOne({
        ...
        include: [
          { model: SkillRole, as: 'skillRoles' },
          { model: SkillGroup, as: 'skills' },
        ],
        transaction: transaction,
      });
  ...
  getDifferenceSkillRoles(dbSkillRoles: any[], inputSkillRoles: any[]) {
    const newSkillRoles = inputSkillRoles.filter(
      (tpl) =>
        dbSkillRoles.find(
          (sk) => sk.userId === tpl.userId && sk.role === tpl.role,
        ) == null,
    );

    const deletedSkillRoles = dbSkillRoles.filter(
      (tpl) =>
        inputSkillRoles.find(
          (sk) => sk.userId === tpl.userId && sk.role === tpl.role,
        ) == null,
    );
  ...
  getDifferenceSkillGroups(dbSkillGroups: any[], inputSkillGroups: any[]) {
    const newSkillGroups = inputSkillGroups.filter(
      (tpl) =>
        dbSkillGroups.find((sk) => sk.departmentId === tpl.departmentId) ==
        null,
    );

    const deletedSkillGroups = dbSkillGroups.filter(
      (tpl) =>
        inputSkillGroups.find((sk) => sk.departmentId === tpl.departmentId) ==
        null,
    );
  ...
  async editProSkill(
    ...
    if (existingName) {
      throw new RuntimeException('IDM_SKILL_NAME_EXIST', HttpStatus.CONFLICT);
    }
    ...
    const inputSkillGroups = [...payload.departments, ...payload.divisions].map(
      (depId) => ({
        departmentId: depId,
        skillId: skill.id,
      }),
    ...
    // ** Optimize by creating and deleting needed relations

    const { newSkillRoles, deletedSkillRoles } = this.getDifferenceSkillRoles(
      dbSkillRoles,
      inputSkillRoles,
    );

    const { newSkillGroups, deletedSkillGroups } =
      this.getDifferenceSkillGroups(dbSkillGroups, inputSkillGroups);

    // ** Start Update
    const transaction = await this.skillEntity.sequelize.transaction();
    try {
      // * Update skill's field
      skill.set('name', payload.skillName.trim());
      await skill.save({ transaction });

      // ** Update skill's relation

      // * Remove deleted relation
      if (deletedSkillRoles.length > 0) {
        await SkillRole.destroy({
          where: {
            skillId: skillId,
            id: { [Op.in]: deletedSkillRoles.map((x) => x.id) },
          },
          transaction,
        });
      }
      ...
      if (deletedSkillGroups.length > 0) {
        await SkillGroup.destroy({
          where: {
            skillId: skillId,
            id: { [Op.in]: deletedSkillGroups.map((x) => x.id) },
          },
          transaction,
        });
      }

      // *  Bulk add new relation
    ...
    // Update skill object to return
    skill.skills = [...payload.departments, ...payload.divisions].map(
      (dep) => ({
        skillId: skill.id,
        departmentId: dep,
      }),
    ) as SkillGroup[];
    skill.skillRoles = [...setters, ...approvers] as SkillRole[];

    return skill;
  }

  async exportHistoryEvaluation(
    division: string,
    department: string,
    userInfo: string,
    status: any,
  ...
  async getListUser(periodId: number, companyGroupCode: string): Promise<any> {
...
// Function
const isAllLowLevel = (currentValue: any[]) => {
  let isLowlevel = true;
...
const summaryPointEvaluator2 = (currentValue: any[]): Number => {
  let totalSum = 0;
  let isNull = false;
  currentValue.forEach((e: any) => {
    totalSum +=
      ((e.level <= 7
        ...
        : e.summaryDepartment?.summaryPointEvaluator2) *
  ...
  currentValue.forEach((e: any) => {
    if (
      (e.level <= 7
        ? e.summaryPointEvaluator2
        : e.summaryDepartment?.summaryPointEvaluator2) === null
    )
      isNull = true;
  });
  ...
  if (dataList.status !== 50) {
    ...
    if (dataList.evaluationPeriod) {
      if (
        ...
        if (
          today >= moment(dataList.dateEvaluationStart).format('YYYY/MM/DD') &&
...
function checkArray(arr) {
  const hasOneToSeven = arr.some((num: number) => num >= 1 && num <= 7);
  const hasEightToTen = arr.some((num: number) => num >= 8 && num <= 10);
