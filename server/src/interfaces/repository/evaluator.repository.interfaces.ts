...
export interface EvaluatorRepositoryI {
  listUserEvaluator(params: EvaluatorSearchInterfaces);
  getNewTransaction();
  ...
  exportHistoryEvaluationEvaluator(
  ...
  getDivDepToExportHistoryEvaluation(
    userId: number,
    companyGroupCode: string,
    params: { yearEvaluate: string; periodEvaluate: string },
  );
  listUserProSkillExpertise(
  ...
  getListDepartmentExpertise(
    userId: number,
    companyGroupCode: string,
    param: any,
  );
  getListEvaluationToExportPDF(
    year: number,
    periodIndex: number,
    userId: number,
    companyGroupCode: string,
