"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.PdfService = void 0;
const common_1 = require("@nestjs/common");
const jspdf_1 = require("jspdf");
const jspdf_autotable_1 = require("jspdf-autotable");
const achievementAdditionalTablePdf_1 = require("../common/business/pdf/achievementAdditionalTablePdf");
const achievementSubTablePdf_1 = require("../common/business/pdf/achievementSubTablePdf");
const achievementTablePdf_1 = require("../common/business/pdf/achievementTablePdf");
const basicTablePdf_1 = require("../common/business/pdf/basicTablePdf");
const proSkillTablePdf_1 = require("../common/business/pdf/proSkillTablePdf");
const Pdf17Helper_1 = require("../common/pdf/Pdf17Helper");
const MsGothic_1 = require("../static/fonts/MS-Gothic/MsGothic");
let PdfService = class PdfService {
    constructor() {
        this.fontName = 'msgothic-normal';
        this.xDefault = 10;
    }
    pdfConfig(orientation = 'p', format = 'a4') {
        const doc = new jspdf_1.jsPDF({
            unit: 'mm',
            orientation,
            format,
            putOnlyUsedFonts: true,
            floatPrecision: this.xDefault,
        });
        const mainFont = MsGothic_1.default;
        doc.addFileToVFS('MS-Gothic.ttf', mainFont);
        doc.addFont('MS-Gothic.ttf', 'msgothic-normal', 'normal');
        doc.setFont('msgothic-normal');
        this.pageWidth = doc.internal.pageSize.width;
        this.pageHeight = doc.internal.pageSize.height;
        return doc;
    }
    headerPdf(doc, titleHeader = 'EVALUATION REPORT', userInfo) {
        const base64Image = '';
        const readImage = new Buffer(base64Image, 'base64');
        doc.addImage(readImage, 'png', 5, 5, 30, 12);
        doc.setFontSize(20);
        this.pageWidth = doc.internal.pageSize.width;
        this.pageHeight = doc.internal.pageSize.height;
        doc.text(titleHeader, this.pageWidth / 2, 30, { align: 'center' });
        if (userInfo) {
            const { employeeNumber, fullName, department, periodStart, periodEnd, companyName, evaluators, level, } = userInfo;
            const x = this.xDefault;
            doc.setFontSize(11);
            doc.text(`従業員名: ${fullName}`, x, 40);
            doc.text(`従業員番号: ${employeeNumber}`, x, 45);
            doc.text(`会社: ${companyName}`, x, 50);
            doc.text(`部署: ${department}`, x, 55);
            doc.text(`評価者: ${(evaluators === null || evaluators === void 0 ? void 0 : evaluators.length) > 0 ? evaluators.join('、') : ''}`, x, 60);
            doc.text(`評価期間: ${periodStart} ～ ${periodEnd}`, x, 65);
            doc.text(`等級: ${level}`, x, 70);
        }
    }
    shortHeaderPdf(doc, titleHeader = 'EVALUATION REPORT', userInfo) {
        const base64Image = '';
        const readImage = new Buffer(base64Image, 'base64');
        doc.addImage(readImage, 'png', 5, 5, 30, 12);
        doc.setFontSize(20);
        this.pageWidth = doc.internal.pageSize.width;
        this.pageHeight = doc.internal.pageSize.height;
        doc.text(titleHeader, this.pageWidth / 2, 30, { align: 'center' });
        if (userInfo) {
            const { employeeNumber, fullName } = userInfo;
            const x = this.xDefault;
            doc.setFontSize(11);
            doc.text(`従業員名: ${fullName}`, x, 40);
            doc.text(`従業員番号: ${employeeNumber}`, x, 45);
        }
    }
    loadTable(doc, startY, columns, body, columnStyles, showHead, styles, fillColor) {
        (0, jspdf_autotable_1.default)(doc, {
            headStyles: {
                halign: 'center',
                valign: 'middle',
                fillColor: fillColor || [209, 209, 209],
                textColor: [0, 0, 0],
            },
            startY,
            margin: { left: this.xDefault, right: this.xDefault },
            theme: 'grid',
            columns,
            body,
            columnStyles: columnStyles,
            styles: Object.assign({ font: this.fontName, lineColor: [162, 162, 162], lineWidth: 0.1 }, styles),
            didDrawPage: (d) => {
                this.pageHeight = d.cursor.y;
            },
            showHead: showHead || 'everyPage',
        });
    }
    templatePdfReport(doc, evaluation) {
        this.headerPdf(doc, 'EVALUATION REPORT', Object.assign({}, evaluation));
        const { status, isEvaluationDate, evaluatorOrderList } = evaluation;
        const isDisplayEvaluator2 = evaluatorOrderList.some((s) => Number(s) === 2) && status > 50;
        const haveSkill = evaluation.flagSkill === 1;
        if (haveSkill) {
            if (this.pageHeight + 27 > doc.internal.pageSize.height) {
                this.pageHeight = 15;
                doc.addPage();
            }
            doc.text('基本スキル', this.xDefault, this.pageHeight + 8);
            this.loadTable(doc, this.pageHeight + 10, basicTablePdf_1.BasicTablePdfBusiness.column(isDisplayEvaluator2), basicTablePdf_1.BasicTablePdfBusiness.dataSources(evaluation.evaluationBasicSkills, status), {
                title: { halign: 'left', cellWidth: 30 },
                content: { halign: 'left' },
                difficulty: { halign: 'center', valign: 'middle', cellWidth: 20 },
                pointEvaluator2: {
                    halign: 'center',
                    valign: 'middle',
                    cellWidth: 20,
                },
            });
            if (this.pageHeight + 27 > doc.internal.pageSize.height) {
                this.pageHeight = 15;
                doc.addPage();
            }
            doc.text('専門スキル', this.xDefault, this.pageHeight + 8);
            this.loadTable(doc, this.pageHeight + 10, proSkillTablePdf_1.ProSkillTablePdfBusiness.column(isDisplayEvaluator2), proSkillTablePdf_1.ProSkillTablePdfBusiness.dataSources(evaluation.proSkillList, status), {
                itemTitle: { halign: 'left', cellWidth: 30 },
                content: {
                    halign: 'left',
                    font: this.fontName,
                },
                difficulty: { halign: 'center', valign: 'middle', cellWidth: 20 },
                totalPointEvaluator2: {
                    halign: 'center',
                    valign: 'middle',
                    cellWidth: 20,
                },
            });
        }
        if (this.pageHeight + 27 > doc.internal.pageSize.height) {
            this.pageHeight = 15;
            doc.addPage();
        }
        doc.text('行動・情意', this.xDefault, this.pageHeight + 8);
        this.loadTable(doc, this.pageHeight + 10, basicTablePdf_1.BasicTablePdfBusiness.column(isDisplayEvaluator2), basicTablePdf_1.BasicTablePdfBusiness.dataSources(evaluation.evaluationBehaviorSkills, status), {
            title: {
                halign: 'left',
                cellWidth: 30,
            },
            content: {
                halign: 'left',
            },
            difficulty: {
                halign: 'center',
                valign: 'middle',
                cellWidth: 20,
            },
            pointEvaluator2: {
                halign: 'center',
                valign: 'middle',
                cellWidth: 20,
            },
        });
        doc.text('個人目標', this.xDefault, this.pageHeight + 8);
        for (const [index, userEvaluationAchievement,] of evaluation.userEvaluationAchievements.entries()) {
            if (userEvaluationAchievement.achievementStatus !== '小計') {
                this.loadTable(doc, index === 0 ? this.pageHeight + 10 : this.pageHeight, achievementTablePdf_1.AchievementPersonalTablePdfBusiness.column2(isDisplayEvaluator2), achievementTablePdf_1.AchievementPersonalTablePdfBusiness.dataSources([
                    userEvaluationAchievement,
                ]), {
                    title: {
                        cellWidth: this.pageWidth * 30 / 100,
                    },
                    achievementValue: {},
                    method: {
                        cellWidth: this.pageWidth * 30 / 100,
                    },
                    weight: {
                        halign: 'center',
                        valign: 'middle',
                        cellWidth: 20,
                    },
                    difficultyEvaluator2: {
                        halign: 'center',
                        valign: 'middle',
                        cellWidth: 20,
                    },
                });
                this.loadTable(doc, this.pageHeight, achievementSubTablePdf_1.AchievementPersonalSubTablePdfBusiness.column(), userEvaluationAchievement.childrens, {
                    coefficient: {
                        halign: 'center',
                        valign: 'middle',
                        cellWidth: 20,
                    },
                }, 'everyPage', {}, '#E8E8E8');
            }
        }
        if (this.pageHeight + 27 > doc.internal.pageSize.height) {
            this.pageHeight = 15;
            doc.addPage();
        }
        if ((status === 50 && isEvaluationDate) || status > 50) {
            doc.text('個人成果', this.xDefault, this.pageHeight + 8);
            this.loadTable(doc, this.pageHeight + 10, achievementTablePdf_1.AchievementPersonalTablePdfBusiness.column(isDisplayEvaluator2), achievementTablePdf_1.AchievementPersonalTablePdfBusiness.dataSources2(evaluation.userEvaluationAchievements), {
                achievementStatus: {
                    halign: 'center',
                    cellWidth: 15,
                },
                reasonComment: {
                    halign: 'left',
                },
                actionPlan: {
                    halign: 'left',
                },
                pointEvaluator2: {
                    halign: 'center',
                    valign: 'middle',
                    cellWidth: 20,
                },
            });
            if (this.pageHeight + 27 > doc.internal.pageSize.height) {
                this.pageHeight = 15;
                doc.addPage();
            }
            if (achievementAdditionalTablePdf_1.AchievementAdditionalTablePdfBusiness.dataSources(evaluation.achievementAdditionals).length > 1) {
                doc.text('追加目標/成果', this.xDefault, this.pageHeight + 8);
                this.loadTable(doc, this.pageHeight + 10, achievementAdditionalTablePdf_1.AchievementAdditionalTablePdfBusiness.column(isDisplayEvaluator2), achievementAdditionalTablePdf_1.AchievementAdditionalTablePdfBusiness.dataSources(evaluation.achievementAdditionals), {
                    titleAdditional: {
                        halign: 'left',
                        cellWidth: 70,
                    },
                    achievementStatus: {
                        halign: 'center',
                        cellWidth: 20,
                    },
                    reasonComment: {
                        halign: 'left',
                    },
                    pointEvaluator2: {
                        halign: 'center',
                        cellWidth: 20,
                    },
                });
            }
            if (this.pageHeight + 27 > doc.internal.pageSize.height) {
                this.pageHeight = 15;
                doc.addPage();
            }
            const comment = evaluation.comment.comment1Public || '該当データがありません';
            doc.text('一次評価者コメント', this.xDefault, this.pageHeight + 10);
            this.loadTable(doc, this.pageHeight + 10, [{ header: '個人目標', dataKey: 'title' }], [{ title: comment }], {}, 'never', { lineWidth: 0 });
            const comment2 = evaluation.comment.comment2Public || '該当データがありません';
            doc.text('二次評価者コメント', this.xDefault, this.pageHeight + 10);
            this.loadTable(doc, this.pageHeight + 10, [{ header: '個人目標', dataKey: 'title' }], [{ title: comment2 }], {}, 'never', { lineWidth: 0 });
        }
        return doc;
    }
    exportEvaluationForPdf(evaluation, orientation = 'p', format = 'a4') {
        const doc = this.pdfConfig(orientation, format);
        const results = this.templatePdfReport(doc, evaluation);
        const arrayBuffer = results.output('arraybuffer');
        return {
            buffer: Buffer.from(arrayBuffer),
            fileName: `【${evaluation.fiscalYear}】${evaluation.fullName}評価表.pdf`,
        };
    }
    getSummaryPeriodTable(evaluations, doc) {
        this.shortHeaderPdf(doc, 'EVALUATION REPORT', Object.assign({}, evaluations[0]));
        let currentY = 50;
        (0, jspdf_autotable_1.default)(doc, {
            theme: 'grid',
            startY: currentY,
            margin: { left: this.xDefault, right: this.xDefault },
            styles: {
                font: this.fontName,
                lineColor: [162, 162, 162],
                lineWidth: 0.1,
            },
            head: [Pdf17Helper_1.Pdf17Helper.getHeaderSummaryPeriodTable()],
            body: Pdf17Helper_1.Pdf17Helper.getSummaryPeriodTableData(evaluations),
            headStyles: { fillColor: [209, 209, 209], textColor: [0, 0, 0] },
        });
        currentY = doc.lastAutoTable.finalY + 10;
        return doc;
    }
    async exportListEvaluationPdf(evaluations, orientation = 'p', format = 'a4') {
        const fileName = `【${evaluations[0].fiscalYear}】${evaluations[0].fullName}評価表.pdf`;
        let doc = this.pdfConfig(orientation, format);
        doc = this.getSummaryPeriodTable(evaluations, doc);
        doc.addPage();
        for (let i = 0; i < evaluations.length; i++) {
            if (i > 0) {
                doc.addPage();
            }
            doc = await this.templatePdfReport(doc, evaluations[i]);
        }
        const arrayBuffer = await doc.output('arraybuffer');
        return {
            buffer: Buffer.from(arrayBuffer),
            fileName: fileName,
        };
    }
};
PdfService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [])
], PdfService);
exports.PdfService = PdfService;
//# sourceMappingURL=pdf.service.js.map