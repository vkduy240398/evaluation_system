"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.Pdf810Service = void 0;
const jspdf_1 = require("jspdf");
const jspdf_autotable_1 = require("jspdf-autotable");
const EvaluationHelper_1 = require("../common/EvaluationHelper");
const Pdf810Helper_1 = require("../common/pdf/Pdf810Helper");
const Pdf810Helper3_1 = require("../common/pdf/Pdf810Helper3");
const MsGothic_1 = require("../static/fonts/MS-Gothic/MsGothic");
const pdf_service_1 = require("./pdf.service");
const achievementSubTablePdf_1 = require("../common/business/pdf/achievementSubTablePdf");
const LEFT_BRACKET = '【';
const RIGHT_BRACKET = '】';
const YEAR = '年';
const SUFFIX = '評価表.pdf';
const MIME_LOGO = 'png';
const BASE64_IMAGE = '';
class Pdf810Service {
    constructor() {
        this.xDefault = 10;
        this.maxCommentLength = 277;
    }
    initDocument(orientation, size) {
        const doc = new jspdf_1.default({
            unit: 'mm',
            orientation: orientation,
            format: size,
            putOnlyUsedFonts: true,
            floatPrecision: this.xDefault,
        });
        const mainFont = MsGothic_1.default;
        doc.addFileToVFS('MS-Gothic.ttf', mainFont);
        doc.addFont('MS-Gothic.ttf', 'msgothic-normal', 'normal');
        doc.setFont('msgothic-normal');
        return doc;
    }
    getPageHeight(doc) {
        return doc.internal.pageSize.height || doc.internal.pageSize.getHeight();
    }
    getPageWidth(doc) {
        return doc.internal.pageSize.width || doc.internal.pageSize.getWidth();
    }
    async exportEvaluationReportPdf(evaluation, listEvalutor, orientation, size, subList) {
        const fileName = this.getFileName(evaluation.title, evaluation.user.fullName);
        let doc = this.initDocument(orientation, size);
        doc = this.getTemplateThreeReportFrame(evaluation, listEvalutor, subList, doc);
        const arrayBuffer = await doc.output('arraybuffer');
        return {
            buffer: Buffer.from(arrayBuffer),
            fileName: fileName,
        };
    }
    getTemplateThreeReportFrame(evaluation, listEvalutor, subList, doc) {
        var _a, _b;
        const pageWidth = this.getPageWidth(doc);
        let currenY = 0;
        const logoData = new Buffer(BASE64_IMAGE, 'base64');
        doc.addImage(logoData, MIME_LOGO, 5, 5, 30, 12.5);
        doc.setFontSize(20);
        doc.text('EVALUATION REPORT', pageWidth / 2, 30, {
            align: 'center',
        });
        currenY += 40;
        doc.setFontSize(11);
        doc.text(`従業員名: ${evaluation.user.fullName}`, this.xDefault, currenY);
        currenY += 5;
        doc.text(`従業員番号: ${evaluation.user.employeeNumber}`, this.xDefault, currenY);
        currenY += 5;
        doc.text(`会社: ${evaluation.companyName}`, this.xDefault, currenY);
        currenY += 5;
        doc.text(`部署: ${evaluation.divisionName}`, this.xDefault, currenY);
        currenY += 5;
        doc.text(`評価者: ${EvaluationHelper_1.EvaluationHelper.getOrderEvaluators(evaluation.evaluator)}`, this.xDefault, currenY);
        currenY += 5;
        doc.text(`評価期間: ${evaluation.periodStart} ～ ${evaluation.periodEnd}`, this.xDefault, currenY);
        currenY += 5;
        doc.text(`等級: ${evaluation.level}`, this.xDefault, currenY);
        currenY += 5;
        if (evaluation.status > 50) {
            (0, jspdf_autotable_1.default)(doc, {
                styles: {
                    font: 'msgothic-normal',
                    lineColor: [162, 162, 162],
                    lineWidth: 0.1,
                },
                margin: { left: this.xDefault, right: this.xDefault },
                theme: 'grid',
                startY: currenY,
                head: Pdf810Helper3_1.Pdf810Helper3.getHeaderSummaryTable(),
                headStyles: { fillColor: [209, 209, 209], textColor: [0, 0, 0] },
                body: Pdf810Helper3_1.Pdf810Helper3.getSummaryTableData(evaluation),
                columnStyles: {
                    0: { cellWidth: 40, halign: 'center' },
                    1: { halign: 'center' },
                    2: { halign: 'center' },
                    3: { halign: 'center' },
                    4: { halign: 'center' },
                },
            });
            currenY = doc.lastAutoTable.finalY + 8;
        }
        if (evaluation.status < 51)
            currenY += 3;
        doc.text(`部門目標`, this.xDefault, currenY);
        currenY += 2;
        if (evaluation.evaluationAchievementPersonals.length) {
            evaluation.evaluationAchievementPersonals.forEach((el) => {
                (0, jspdf_autotable_1.default)(doc, {
                    theme: 'grid',
                    styles: {
                        font: 'msgothic-normal',
                        lineColor: [162, 162, 162],
                        lineWidth: 0.1,
                    },
                    margin: { left: this.xDefault, right: this.xDefault },
                    startY: currenY,
                    columns: Pdf810Helper3_1.Pdf810Helper3.getHeaderDepartmentGoalTable(evaluation.status),
                    columnStyles: {
                        0: { cellWidth: pageWidth * 29 / 100, },
                        1: { cellWidth: 'auto' },
                        2: { cellWidth: pageWidth * 30 / 100 },
                        3: { halign: 'center', cellWidth: 20 },
                        4: { halign: 'center', cellWidth: 20 },
                    },
                    headStyles: {
                        valign: 'middle',
                        halign: 'center',
                        fillColor: [209, 209, 209],
                        textColor: [0, 0, 0],
                    },
                    body: Pdf810Helper3_1.Pdf810Helper3.getDepartmentGoalTableData(el),
                });
                currenY = doc.lastAutoTable.finalY;
                (0, jspdf_autotable_1.default)(doc, {
                    theme: 'grid',
                    styles: {
                        font: 'msgothic-normal',
                        lineColor: [162, 162, 162],
                        lineWidth: 0.1,
                    },
                    margin: { left: this.xDefault, right: this.xDefault },
                    startY: currenY,
                    columns: achievementSubTablePdf_1.AchievementPersonalSubTablePdfBusiness.column(),
                    columnStyles: {
                        0: { cellWidth: 'auto' },
                        1: { halign: 'center', cellWidth: 20, valign: 'middle' },
                    },
                    headStyles: {
                        valign: 'middle',
                        halign: 'center',
                        fillColor: '#E8E8E8',
                        textColor: [0, 0, 0],
                    },
                    body: Pdf810Helper3_1.Pdf810Helper3.getSubListData(el, subList),
                });
                currenY = doc.lastAutoTable.finalY;
            });
        }
        currenY = doc.lastAutoTable.finalY + 8;
        if (evaluation.status > 50) {
            doc.text(`部門成果`, this.xDefault, currenY);
            currenY += 2;
            (0, jspdf_autotable_1.default)(doc, {
                theme: 'grid',
                styles: {
                    font: 'msgothic-normal',
                    lineColor: [162, 162, 162],
                    lineWidth: 0.1,
                },
                margin: { left: this.xDefault, right: this.xDefault },
                startY: currenY,
                columns: Pdf810Helper3_1.Pdf810Helper3.getHeaderDepartmentAchievementTable(),
                columnStyles: {
                    0: { cellWidth: 19, halign: 'center' },
                    1: { minCellWidth: 65 },
                    2: { minCellWidth: 70 },
                    3: { halign: 'center', cellWidth: 18 },
                    4: { halign: 'center', cellWidth: 18 },
                },
                headStyles: {
                    valign: 'middle',
                    halign: 'center',
                    fillColor: [209, 209, 209],
                    textColor: [0, 0, 0],
                },
                body: Pdf810Helper_1.Pdf810Helper.getDepartmentAchievementTableData(evaluation),
            });
            currenY = doc.lastAutoTable.finalY + 8;
            if (Pdf810Helper_1.Pdf810Helper.getAdditionalTableData(evaluation).length > 1) {
                doc.text(`追加目標/成果`, this.xDefault, currenY);
                currenY += 2;
                (0, jspdf_autotable_1.default)(doc, {
                    theme: 'grid',
                    styles: {
                        font: 'msgothic-normal',
                        lineColor: [162, 162, 162],
                        lineWidth: 0.1,
                    },
                    margin: { left: this.xDefault, right: this.xDefault },
                    startY: currenY,
                    columns: Pdf810Helper3_1.Pdf810Helper3.getHeaderAdditionalTable(),
                    columnStyles: {
                        0: { cellWidth: 70 },
                        1: { cellWidth: 30, halign: 'center' },
                        2: { cellWidth: 70 },
                        3: { halign: 'center' },
                    },
                    headStyles: {
                        valign: 'middle',
                        halign: 'center',
                        fillColor: [209, 209, 209],
                        textColor: [0, 0, 0],
                    },
                    body: Pdf810Helper_1.Pdf810Helper.getAdditionalTableData(evaluation),
                });
            }
            currenY = doc.lastAutoTable.finalY + 8;
            const comment1Info = listEvalutor.filter((item) => {
                return item.evaluationOrder.toString() === '1.0';
            });
            const comment2Info = listEvalutor.filter((item) => {
                return item.evaluationOrder.toString() === '2.0';
            });
            doc.text(`一次評価者コメント`, this.xDefault, currenY);
            currenY += 2;
            (0, jspdf_autotable_1.default)(doc, {
                theme: 'grid',
                styles: {
                    font: 'msgothic-normal',
                    lineWidth: 0,
                },
                margin: { left: this.xDefault, right: this.xDefault },
                startY: currenY,
                body: [
                    {
                        content: ((_a = comment1Info[0]) === null || _a === void 0 ? void 0 : _a.commentPublic) || '該当データがありません',
                    },
                ],
                showHead: 'never',
            });
            currenY = doc.lastAutoTable.finalY + 8;
            doc.text(`二次評価者コメント`, this.xDefault, currenY);
            currenY += 2;
            (0, jspdf_autotable_1.default)(doc, {
                theme: 'grid',
                styles: {
                    font: 'msgothic-normal',
                    lineWidth: 0,
                },
                margin: { left: this.xDefault, right: this.xDefault },
                startY: currenY,
                body: [
                    {
                        content: ((_b = comment2Info[0]) === null || _b === void 0 ? void 0 : _b.commentPublic) || '該当データがありません',
                    },
                ],
                showHead: 'never',
            });
            currenY = doc.lastAutoTable.finalY + 8;
        }
        return doc;
    }
    getSummaryPeriodTable(evaluations, doc) {
        const pageWidth = this.getPageWidth(doc);
        let currenY = 0;
        const logoData = new Buffer(BASE64_IMAGE, 'base64');
        doc.addImage(logoData, MIME_LOGO, 5, 5, 30, 12.5);
        doc.setFontSize(20);
        doc.text('EVALUATION REPORT', pageWidth / 2, 30, {
            align: 'center',
        });
        currenY += 40;
        doc.setFontSize(11);
        doc.text(`従業員名: ${evaluations[0].user.fullName}`, this.xDefault, currenY);
        currenY += 5;
        doc.text(`従業員番号: ${evaluations[0].user.employeeNumber}`, this.xDefault, currenY);
        currenY += 5;
        (0, jspdf_autotable_1.default)(doc, {
            styles: {
                font: 'msgothic-normal',
                lineColor: [162, 162, 162],
                lineWidth: 0.1,
            },
            margin: { left: this.xDefault, right: this.xDefault },
            theme: 'grid',
            startY: currenY,
            head: Pdf810Helper_1.Pdf810Helper.getHeaderSummaryTable(),
            body: Pdf810Helper_1.Pdf810Helper.getSummaryPeriodTableData(evaluations),
            columnStyles: {
                0: { halign: 'center' },
                1: { halign: 'center' },
                2: { halign: 'center' },
            },
            headStyles: {
                valign: 'middle',
                halign: 'center',
                fillColor: [209, 209, 209],
                textColor: [0, 0, 0],
            },
        });
        currenY = doc.lastAutoTable.finalY + 10;
        return doc;
    }
    getFileName(title, fullname) {
        const fileName = LEFT_BRACKET + title + RIGHT_BRACKET + fullname + SUFFIX;
        return fileName;
    }
    async getChildEvaluation810ReportPdf(evaluation, listEvalutor, role, userId, subList, doc) {
        doc = await this.getTemplateThreeReportFrame(evaluation, listEvalutor, subList, doc);
        return doc;
    }
    async exportParentReportPdf(evaluations, role, userId, orientation = 'p', size = 'a4', subList) {
        const fileName = `【${evaluations[0].title}】${evaluations[0].user.fullName}評価表.pdf`;
        let doc = this.initDocument(orientation, size);
        if (evaluations.length !== 1) {
            doc = await this.getSummaryPeriodTable(evaluations, doc);
            doc.addPage();
        }
        for (let i = 0; i < evaluations.length; i++) {
            if (i > 0) {
                doc.addPage();
            }
            doc = await this.getChildEvaluation810ReportPdf(evaluations[i], evaluations[i].evaluator, role, userId, subList, doc);
        }
        const arrayBuffer = await doc.output('arraybuffer');
        return {
            buffer: Buffer.from(arrayBuffer),
            fileName: fileName,
        };
    }
    async exportListPDFReport(evaluations, role, userId, orientation = 'p', size = 'a4', subList) {
        var _a;
        let fileName = ``;
        if (((_a = evaluations[0]) === null || _a === void 0 ? void 0 : _a.user) && evaluations[0].user.fullName) {
            fileName = `【${evaluations[0].fiscalYear}】${evaluations[0].user.fullName}評価表.pdf`;
        }
        else {
            fileName = `【${evaluations[0].fiscalYear}】${evaluations[0].fullName}評価表.pdf`;
        }
        let doc = this.initDocument(orientation, size);
        const pdfService17 = new pdf_service_1.PdfService();
        for (let i = 0; i < evaluations.length; i++) {
            if (i > 0) {
                doc.addPage();
            }
            if (evaluations[i].level < 8) {
                doc = await pdfService17.templatePdfReport(doc, evaluations[i]);
            }
            else {
                doc = await this.getChildEvaluation810ReportPdf(evaluations[i], evaluations[i].evaluator, role, userId, subList, doc);
            }
        }
        const arrayBuffer = await doc.output('arraybuffer');
        return {
            buffer: Buffer.from(arrayBuffer),
            fileName: fileName,
        };
    }
}
exports.Pdf810Service = Pdf810Service;
//# sourceMappingURL=pdf810.service.js.map